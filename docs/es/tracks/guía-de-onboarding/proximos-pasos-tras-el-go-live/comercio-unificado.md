...
- Gestión de promociones específicas para cada cuenta sin la obligación de contratar una nueva [política comercial](/es/docs/tutorials/como-funciona-una-politica-comercial).

- Posibilidad de pago en el entorno de la cuenta principal o de la cuenta franquicia ([split de pago](/es/docs/tutorials/split-de-pagos)/pedido).

- [Regionalización de sellers](/es/docs/tutorials/configurar-la-regionalizacion-del-seller) (precio y stock).
...
> ⚠️ Las [cuentas franquicia](/es/docs/tutorials/que-es-una-cuenta-franquicia) actúan como un [seller white label](/es/docs/tutorials/seller-white-label) de la cuenta principal. Cuando un cliente agrega productos al carrito y simula el envío en el sitio web de la cuenta principal, el [algoritmo de selección de sellers white label](/es/docs/tutorials/seleccion-de-sellers-white-label) asigna automáticamente la cuenta franquicia que se encargará del [fulfillment](/es/docs/tutorials/fulfillment-logistica-vtex) del pedido. Si uno o más productos del carrito no están disponibles en la cuenta franquicia seleccionada, el sistema asignará otra cuenta disponible para realizar el envío de los productos faltantes, lo que permite que haya más de una entrega para el mismo pedido. Este proceso se lleva a cabo internamente, sin que el cliente vea los sellers.
