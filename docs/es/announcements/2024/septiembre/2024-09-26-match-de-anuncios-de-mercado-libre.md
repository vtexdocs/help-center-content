---
title: 'Match de anuncios de Mercado Libre'
id: 1RZPtcsPMSZd2VbNkvAqz9
status: PUBLISHED
createdAt: 2024-09-26T14:22:17.927Z
updatedAt: 2024-09-27T03:00:02.843Z
publishedAt: 2024-09-27T03:00:02.843Z
contentType: updates
productTeam: Channels
author: 2p7evLfTcDrhc5qtrzbLWD
slugEN: 2024-09-26-mercado-libre-offer-match
locale: es
legacySlug: match-de-anuncios-de-mercado-libre
announcementImageID: 'undefined'
announcementSynopsisES: 'Vincula tus anuncios a las oportunidades del catálogo de Mercado Libre para que tus productos sean más relevantes.'
---

Al realizar la [integración](/es/tracks/configurar-integracao-do-mercado-livre--2YfvI3Jxe0CGIKoWIGQEIq) con Mercado Libre, el seller envía su catálogo al marketplace. Con los productos enviados y los anuncios creados, Mercado Libre ofrece al seller oportunidades de match con anuncios de tipo **catálogo**, que pueden ocupar un espacio resaltado en el marketplace. Para analizar una oportunidad, el seller debe acceder al Admin VTEX **Marketplace > Mercado Libre > Match de anuncios** o ingresar **Match de anuncios**.  

## ¿Cuáles son los beneficios de esta funcionalidad?

Los principales beneficios de la funcionalidad son:

- **Mayor visibilidad:** la asociación con el **catálogo de Mercado Libre** sitúa a tus productos en un espacio resaltado, lo que puede traducirse en mejores posiciones en las búsquedas de los clientes.  
- **Gestión simplificada:** la pestaña **Oportunidades** permite ver, filtrar y buscar anuncios elegibles de forma práctica e intuitiva.  
- **Flexibilidad:** posibilidad de analizar y vincular oportunidades tanto individualmente como en masa, según las necesidades del seller.  
- **Control y monitoreo:** la pestaña **Catálogos activos** permite realizar un seguimiento detallado de los anuncios ya vinculados, facilitando acciones rápidas para mejorar o mantener la relevancia.  

## ¿Qué se necesita hacer?

Accede a la funcionalidad **Match de anuncios** en el Admin VTEX haciendo clic en **Marketplace > Mercado Libre > Match de anuncios** o ingresa  Match de anuncios en la barra de búsqueda. Está disponible para todas las cuentas VTEX.

Para más información sobre esta funcionalidad, consulta el artículo [Match de anuncios de Mercado Libre](/es/tracks/configurar-integracao-do-mercado-livre--2YfvI3Jxe0CGIKoWIGQEIq/43uD4LPU5PLUWe11IaWwyR). 

