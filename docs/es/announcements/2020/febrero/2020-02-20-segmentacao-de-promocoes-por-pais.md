---
title: 'Segmentación de promociones por país'
id: 1e2kgDZ4yaAIaatCANIA0G
status: PUBLISHED
createdAt: 2020-02-20T13:51:37.669Z
updatedAt: 2020-11-27T19:39:59.173Z
publishedAt: 2020-04-02T16:44:45.173Z
contentType: updates
productTeam: Marketing & Merchandising
author: 7qy2DBsUp8U5P9lqV0JHfR
slugEN: 2020-02-20-promotions-segmentation-by-country
locale: es
legacySlug: segmentacao-de-promocoes-por-pais
announcementImageID: 'undefined'
announcementSynopsisES: 'VTEX presenta una nueva feature para los clientes que operan en varios países: el filtro de promoción por país.'
---

A partir del día 21 de Febrero de 2020, VTEX presenta una nueva feature para los clientes que operan en varios países: el filtro de promoción por país.

Esta funcionalidad permite que el usuario segmente el uso de una promoción para los países que reciben entregas de su tienda.

Hecho esto, las localidades se mostrarán en el módulo Promociones del Admin de su tienda.

Para saber más detalles sobre Promociones, vea nuestro [track](/es/docs/tracks/como-funcionan-las-promociones).
