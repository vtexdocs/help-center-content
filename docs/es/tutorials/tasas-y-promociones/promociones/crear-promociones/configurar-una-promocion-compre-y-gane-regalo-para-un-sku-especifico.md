---
title: 'Crear una promoción Comprar y Obtener (regalo) para un SKU específico'
id: 4QlFtwxAju6qOwaCi2YS0G
status: PUBLISHED
createdAt: 2017-08-22T15:24:13.195Z
updatedAt: 2023-03-31T17:05:27.785Z
publishedAt: 2022-03-03T17:07:36.574Z
firstPublishedAt: 2017-08-22T15:41:28.450Z
contentType: tutorial
productTeam: Marketing & Merchandising
author: authors_84
slugEN: how-to-set-a-buy-and-win-promotion-gift-for-a-specific-sku
legacySlug: configurar-una-promocion-compre-y-gane-regalo-para-un-sku-especifico
locale: es
subcategoryId: 1yTYB5p4b6iwMsUg8uieyq
---

La configuración de esta promoción se hace por el tipo __Comprar y Obtener__, en el que es posible seleccionar qué SKU dará el regalo.

1. En el módulo __Taxas y promociones__, acceda __Promociones__.
2. Haga clic en el botón `Nueva Promoción`.
3. Haga clic en la opción `Comprar y Obtener`.
4. Rellene los campos de conformidad con el artículo [Cómo funciona la promoción Compre y gane
](/es/docs/tutorials/como-funciona-la-promocion-compre-y-gane).
5. En el campo **Qué tipo y importe de descuento?**, seleccione la opción **Regalo** y rellene en campo seleccionando el SKU que desea conceder la promoción con regalo.
7. Existe la posibilidad de **Activar multiplicador de regalo** para definir si será un regalo por la compra total o si será un regalo por cada SKU comprado. 
8. Haga clic en el botón `Salvar`.

Esta promoción hará que el regalo aparezca en la página del producto.
