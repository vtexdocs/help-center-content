...
Para importar productos y SKUs a partir de una plantilla sigue los pasos a continuación:
 ...
 5. Llena los datos de la plantilla, tal como se indica en la sección [Campos de la plantilla de productos y SKUs](#campos-de-la-plantilla-de-productos-y-skus).
  ...
  > ⚠️ El límite de la plantilla de importación es de 10.000 líneas.
...
3. En la parte superior de la página, haz clic en `Exportar`.
  ...
  > ⚠️ El límite de exportación es de 300.000 productos.
...
![export-es-loading](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/catalogo/importacion-y-exportacion/importar-y-exportar-productos-y-skus-mediante-plantilla_1.png)
...
![export-es-done](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/catalogo/importacion-y-exportacion/importar-y-exportar-productos-y-skus-mediante-plantilla_2.png)
...
| Mostrar en el sitio web* | Si deseas que el producto sea visible en la vitrina, llena con `sí`. Si prefieres que se mantenga oculto, llena con `no`. Este campo acepta los siguientes valores: true, TRUE, yes, YES, sim, SIM, si, SI, sí, SÍ / false, FALSE, no, NO, não, NÃO, nao, NAO. | sí |
| Mostrar cuando no hay stock* | Si deseas que el producto sin stock se muestre en la vitrina, llena con `sí`. Esto permite utilizar la opción [Avísame](https://help.vtex.com/es/docs/tutorials/configurar-la-opcion-avisame) para que el cliente ingrese su email y recibe una notificación cuando el producto esté disponible nuevamente. Si prefieres que el producto sin stock se oculte en la vitrina, llena con `no`. Este campo acepta los siguientes valores: true, TRUE, yes, YES, sim, SIM, si, SI, sí, SÍ / false, FALSE, no, NO, não, NÃO, nao, NAO. | no |
...
| Activar SKU si es posible* | Si deseas la activación automática del SKU después de verificar los [requisitos de disponibilidad](/es/faq/por-que-el-producto-no-aparece-en-el-sitio-web), llena con `sí`. Si no deseas que se active, llena con `no`. Este campo acepta los siguientes valores: true, TRUE, yes, YES, sim, SIM, si, SI, sí, SÍ / false, FALSE, no, NO, não, NÃO, nao, NAO. | sí |
| SKU Activo | Este campo define si el SKU está activo (`sí`) o inactivo (`no`). El campo no permite modificación y solo se utiliza para consulta. Este campo acepta los siguientes valores: true, TRUE, yes, YES, sim, SIM, si, SI, sí, SÍ / false, FALSE, no, NO, não, NÃO, nao, NAO. | no |
