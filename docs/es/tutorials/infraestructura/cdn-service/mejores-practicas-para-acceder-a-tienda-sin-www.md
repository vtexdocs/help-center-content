...
* [Cómo funciona la configuración de DNS en VTEX](/es/docs/tutorials/como-funciona-la-configuracion-de-dns-en-vtex)
...
Al [configurar el dominio principal](/es/docs/tutorials/configurar-dominio-de-la-tienda) de la tienda, VTEX recomienda utilizar siempre `www` u otro subdominio, para integrar la necesidad de utilizar CNAME junto a otras configuraciones. En este escenario, se recomienda utilizar redirecciones para acceder a la dirección web sin `www` u otros subdominios. Para saber cómo configurar redirecciones, consulta [Redirección de otras direcciones](/es/docs/tutorials/redireccion-de-otras-direcciones).
...
> ⚠️ Cuando se utiliza la solución DNS Flattening, se comportará de la misma forma que un [proxy inverso](/es/docs/tutorials/insertar-proxy-inverso-frente-a-los-servicios-vtex). En este caso, se debe configurar una entrada de verificación TXT.
