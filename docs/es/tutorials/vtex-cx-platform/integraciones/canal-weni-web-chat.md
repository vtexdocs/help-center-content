---
title: "Canal Weni web chat"
id: 3Y0MyNe0kPEngpYOJG1qpo
status: PUBLISHED
createdAt: 2025-09-15T14:12:38.155Z
updatedAt: 2025-10-03T14:20:33.705Z
publishedAt: 2025-10-03T14:20:33.705Z
firstPublishedAt: 2025-10-03T14:20:33.705Z
contentType: tutorial
productTeam: Post-purchase
author: 4JJllZ4I71DHhIOaLOE3nz
slugEN: web-chat-channel
legacySlug: canal-weni-web-chat
locale: es
subcategoryId: 7e9LaCixVcvflzgWkxlftd
---

Si tienes un sitio web y te gustaría integrar tu bot en él, la solución es nuestro canal **Web chat**, que sirve para atender a las personas que visitan tu sitio y desean obtener más información o asistencia humana.

## Cómo crear un canal web chat

1. En tu proyecto, haz clic en **Aplicaciones > Descubrimientos > Añade el canal Weni Web Chat.**

![](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/vtex-cx-platform/integraciones/canal-weni-web-chat_1.png)

2. A continuación, haz clic en **Mis Aplicaciones** y accede al canal **Web Chat** que has creado.

![](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/vtex-cx-platform/integraciones/canal-weni-web-chat_2.png)

3. Al hacer clic en la aplicación, se abrirá una ventana de configuración de tu **Weni Web Chat** con opciones de personalización para tu Web Chat.

![](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/vtex-cx-platform/integraciones/canal-weni-web-chat_3.png)

**Ajustes**: Aquí puedes asignar un nombre al bot, el nombre de la empresa, además de una palabra clave que activará un flujo de preferencia, un mensaje inicial y otras configuraciones relacionadas con los mensajes.

![](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/vtex-cx-platform/integraciones/canal-weni-web-chat_4.png)

**Apariencia**: puedes cargar un CSS personalizado, una imagen del bot y ajustar la paleta de colores.

![](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/vtex-cx-platform/integraciones/canal-weni-web-chat_5.png)

Al finalizar la personalización de tu **Web Chat**, haz clic en el botón **Guardar cambios** para guardar las modificaciones y generar el código **script**.- **Código Script**: Después de guardar la información, se generará un código **script** que deberás utilizar en tu sitio web.

![](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/vtex-cx-platform/integraciones/canal-weni-web-chat_6.png)
