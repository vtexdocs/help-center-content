...
- **Nativa:** La VTEX CX Platform puede integrarse con APIs que tienen formato REST/Json mediante una tarjeta Webhook o a través de una aplicación preexistente en el menú de Integraciones.
...
Es una forma de comunicación entre sistemas, en la que uno de ellos proporciona información y servicios que pueden ser utilizados por el otro (VTEX CX Platform). Normalmente, una API tiene:
...
Un Webhook es una tecnología utilizada para permitir la comunicación entre dos aplicaciones en tiempo real. En la VTEX CX Platform, podemos buscar e incluso actualizar información en otros sistemas a través de la tarjeta de **Llamar Webhook**. Esto significa que, para consumir una API, utilizarás una tarjeta Webhook:
