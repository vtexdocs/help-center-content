---
title: '¿Qué son estantes?'
id: 28D8d6GFfuAsuAoeWC8eq0
status: PUBLISHED
createdAt: 2018-01-30T22:11:34.468Z
updatedAt: 2023-03-29T14:01:04.454Z
publishedAt: 2023-03-29T14:01:04.454Z
firstPublishedAt: 2018-02-02T18:02:00.341Z
contentType: tutorial
productTeam: Post-purchase
author: 245tA425AIeioKAk2eaiwS
slugEN: what-are-shelves
legacySlug: que-son-estantes
locale: es
subcategoryId: 2g6LxtasS4iSeGEqeYUuGW
---

<div style="background-color:#FCF8F2; border-left: 2px solid #F0AD4E; border-top-left-radius: 2px; border-bottom-left-radius: 2px; padding: 15px; margin-bottom: 10px">
Esta funcionalidad está disponible para las tiendas que utilizan la tecnología Portal Legada.
</div>

Estantes son elementos configurables que muestran una agrupación de productos en las páginas de su web site. Esta agrupación se define a partir de los criterios que usted elija.

Los estantes se estructuran a partir de código HTML, organizados en [templates](/es/docs/tutorials/que-son-templates) específicos, llamados Templates de Estante ("__Shelves Templates__"). Se encuentran en la carpeta **CMS**, en el [módulo Layout](/es/docs/tutorials/que-es-el-cms-layout). Es en esta carpeta que usted podrá crear nuevos templates y editar los que ya existen.

![Shelves Templates](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/storefront/cms-portal-legado/layout/que-son-estantes_1.png)

Por ser un elemento que puede ser utilizado en más de una página, este tipo especial de template (Template de Estante) se almacena en carpetas diferentes de los otros tipos (HTML Templates y Sub Templates). Así, es posible mantener la organización de los archivos y, consecuentemente, del código de su web site.

### Artículos relacionados
- [¿Qué es el CMS?](/es/docs/tutorials/que-es-el-cms-layout)
- [¿Qué es un sitio web?](/es/docs/tutorials/que-es-un-web-site)
- [¿Qué son templates?](/es/docs/tutorials/que-son-templates)
