...
En cambio, el __adquirente__ recibe la información del pago, la procesa y repasa a la [bandera](/es/docs/tutorials/que-es-una-bandera-de-tarjeta-de-credito) (cuando el medio de pago es tarjeta de crédito) y al [banco emisor](/es/docs/tutorials/que-es-el-banco-emisor).

Para que su tienda VTEX pueda recibir pagos, usted necesita habilitar su comunicación con un __adquirente__. Esto se realiza a través de un [gateway](/es/docs/tutorials/que-es-un-gateway-de-pagos), que debe configurarse para procesar las [condiciones de pago deseadas](/es/docs/tutorials/condiciones-de-pago). Así, las compras pueden seguir el flujo de aprobación normalmente.
