...
En el flujo de pedidos hay un [periodo de cancelación](/es/docs/tutorials/flujo-y-status-de-pedidos) (o periodo de gracia) en el que el cliente puede cancelar un pedido automáticamente. Excepto durante ese periodo, el seller puede denegar una solicitud de cancelación del pedido, independientemente de si ha sido realizada por el cliente o por el marketplace.
...
> ℹ️ En este artículo, los términos marketplace y seller se refieren a [marketplace VTEX](/es/docs/tutorials/estrategias-de-marketplace-en-vtex#being-a-vtex-marketplace) y [seller VTEX](/es/docs/tutorials/estrategias-de-marketplace-en-vtex#being-a-vtex-seller). La funcionalidad de denegar solicitudes de cancelación de pedidos no está disponible para sellers o marketplaces externos.
...
![cancel_api_improvements_help_center_ES_v2](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/pedidos/todos-los-pedidos/denegar-solicitud-de-cancelacion-del-pedido_1.png)

La descripción de cada paso es la siguiente:

1. El cliente se pone en contacto con el marketplace para cancelar un pedido o lo hace a través de [Mi cuenta](/es/docs/tutorials/como-funciona-mi-cuenta).
2. El marketplace envía una solicitud de cancelación al seller. En este punto, el [status del pedido](/es/docs/tutorials/flujo-y-status-de-pedidos) para el marketplace es `esperando decisión del seller`.
...
4. El seller recibe la **Solicitud de cancelación** del pedido y decide si la acepta o no. Los próximos pasos informan lo que ocurre cuando el seller decide no cancelar el pedido.
  ...
  > ❗ Para denegar una solicitud de cancelación, el status del pedido para marketplace y seller deben ser los siguientes: <ul> <li>**Status del marketplace:** esperando decisión del seller.</li> <li>**Status del seller:** cancelación solicitada.</li> </ul>
...
La solicitud de cancelación de un pedido puede realizarse por el marketplace o el propio cliente a través de **Mi cuenta.** Cuando se realiza directamente a través de [Mi cuenta](/es/docs/tutorials/como-funciona-mi-cuenta), el cliente recibe un mensaje de confirmación de que la solicitud se ha enviado correctamente.

Después, el [status del pedido](/es/docs/tutorials/flujo-y-status-de-pedidos) se mostrará de la siguiente manera: `cancelación solicitada` para el marketplace y `esperando decisión del seller` para el seller, hasta que este tome una decisión, ya sea para aceptar o denegar la solicitud.
...
1. En el Admin VTEX, ve a **Pedidos > Todos los pedidos,** o ingresa [Todos los pedidos](/es/docs/tutorials/todos-los-pedidos) en la barra de búsqueda de la parte superior de la página.
2. Selecciona el pedido deseado para acceder a la [página de información del pedido](/es/docs/tutorials/pagina-de-detalles-del-pedido).
3. En la parte superior de la página, haz clic en `Denegar`.

  ![deny cancel request](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/pedidos/todos-los-pedidos/denegar-solicitud-de-cancelacion-del-pedido_2.png)

4. Ingresa en el campo Solicitud de cancelación el motivo por el que se deniega la solicitud de cancelación del pedido.

  ![deny cancel request reason](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/es/tutorials/pedidos/todos-los-pedidos/denegar-solicitud-de-cancelacion-del-pedido_3.png)

  > ⚠️ Atención: el cliente puede recibir el mensaje que ingreses en este campo a través de [Mi cuenta](/es/docs/tutorials/como-funciona-mi-cuenta) o a través del marketplace.
