---
title: '¿Qué hacer cuando el NSU del producto se duplica en las transacciones realizadas por Rede?'
excerpt: "Quien emite el NSU es Rede, no VTEX. Si dos transacciones comparten el NSU, concilie por el TID y contacte a Rede para corregir la duplicidad."
id: 6GCSJnX9pnCk8jVOQ4cDK1
status: PUBLISHED
createdAt: 2019-02-27T20:58:12.700Z
updatedAt: 2019-12-31T14:24:44.752Z
publishedAt: 2019-12-31T14:24:44.752Z
firstPublishedAt: 2019-02-27T21:00:00.706Z
contentType: frequentlyAskedQuestion
productTeam: Payments
author: 10Jj3Ca0hIsTVp4BEeef6Q
slugEN: what-to-do-when-the-products-nsu-is-duplicated-in-transactions-performed-by-rede
locale: es
legacySlug: 
---

Eventualmente, el [NSU](/es/docs/tutorials/como-encontrar-nsu-y-tid-del-pedido), o Número Secuencial Único, se puede duplicar en el adquirente Rede. Los errores como éste pueden perturbar el proceso de conciliación.

Sin embargo, en circunstancias en que el NSU es el mismo para dos clientes diferentes, una alternativa para verificar la transacción es dar baja en el pago utilizando los números de [TID](/es/docs/tutorials/como-visualizar-detalle-del-pedido), que son distintos entre sí.

Atención: es importante resaltar que el NSU no es generado por VTEX, sino por el adquirente responsable de la transacción. Por lo tanto, es importante ponerse en contacto con el equipo de e-commerce de Rede para resolver el problema en conjunto.
