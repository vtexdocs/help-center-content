---
title: "Aplicar preços manuais em itens de assinaturas"
id: 7vyYh0mDWkvwqMybqjpgHt
status: PUBLISHED
createdAt: 2022-05-10T19:23:46.393Z
updatedAt: 2022-05-10T19:44:14.836Z
publishedAt: 2022-05-10T19:44:14.836Z
contentType: updates
productTeam: Post-purchase
author: 1malnhMX0vPThsaJaZMYm2
slug: aplicar-precos-manuais-em-itens-de-assinaturas
legacySlug: aplicar-precos-manuais-em-itens-de-assinaturas
announcementImageID: ""
announcementSynopsisPT: Agora você pode habilitar preços manuais para assinaturas em sua loja.
---

Ao utilizar [Assinaturas](https://help.vtex.com/pt/tutorial/como-funciona-a-assinatura--frequentlyAskedQuestions_4453), aplicativo desenvolvido pela VTEX para facilitar vendas recorrentes, é possível que os comerciantes precisem configurar um preço manual para cada item a fim de oferecer melhores ofertas aos compradores recorrentes. 

Pensando nisso, agora você pode habilitar a funcionalidade de Preço Manual para Assinaturas v3 na sua loja, que permite:

* Aplicar um preço manual em cada item de assinatura, em vez de ter o preço atual aplicado.
* Manter o mesmo preço manual para cada pedido recorrente futuro dessa assinatura, se desejar.

## O que mudou?

Anteriormente, a funcionalidade de Preço Manual não estava disponível para pedidos de assinatura, somente para pedidos comuns durante o Checkout. Agora, ampliamos o alcance dessa funcionalidade para incluir também o Assinaturas v3.

## Por que fizemos esta mudança?

Esta alteração foi motivada pela demanda dos lojistas de estabelecer preços manuais para cada item em um pedido de assinatura. Considerando que os preços dos produtos podem mudar ao longo do tempo, manter as mesmas regras comerciais para pedidos futuros de assinaturas pode ser uma boa estratégia para oferecer preços competitivos e reter os clientes por mais tempo.

## O que precisa ser feito?

Para habilitar a funcionalidade de Preço Manual para Assinaturas, siga os passos descritos no guia para desenvolvedores [Enabling Manual Prices for Subscriptions v3](https://developers.vtex.com/vtex-rest-api/docs/enabling-manual-prices-for-subscriptions-v3) (Habilitar Preços Manuais para Assinaturas v3).

<div class="alert alert-warning">
<p>A funcionalidade de Preço Manual está disponível apenas para Assinaturas v3. Se você estiver utilizando uma versão anterior do sistema de Assinaturas, consulte o <a href="https://developers.vtex.com/vtex-rest-api/docs/subscriptions-v3-migration-guide">Subscriptions v3 migration guide</a> (Guia de migração para Assinaturas v3).</p>
</div>
