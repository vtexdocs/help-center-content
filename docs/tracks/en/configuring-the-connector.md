...
3. Select `Settings`.
4. On the Google Shopping card, click on `Integrate`.
5. Fill in the fields in the modal window (fields marked with an asterisk are mandatory). Then click on the `Save configuration` button. Below you can see a short description of each field to guide you.
...
- **Sales Policy***: number of the trade policy that will define the catalog assortment and the price of the items used in the integration.
