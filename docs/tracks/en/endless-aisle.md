...
To do this, follow the steps in the article [Managing physical stores and sales associates in inStore](https://help.vtex.com/en/tracks/instore-setting-up--zav76TFEZlAjnyBVL5tRc/5PSjRstg7UU4lOm0s8aqKN).
...
If the store's main Trade Policy is also used to sell on marketplaces, you must use a different Trade Policy for VTEX Sales App. Otherwise, the payment methods `Direct Selling Debit` and `Direct Selling Credit`, configured for inStore, will also be displayed at the checkout of marketplaces.

If, on the other hand, your store uses a Trade Policy dedicated to inStore, make sure that:
- Your products are priced according to this dedicated Trade Policy.
- The dedicated Trade Policy is applied to the products that should serve the store where inStore will be used.
