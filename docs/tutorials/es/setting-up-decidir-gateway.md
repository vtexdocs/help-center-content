---
title: 'Configurar gateway Decidir'
id: 4QvUUoYubmQu2e8AQqu2es
status: DRAFT
createdAt: 2018-04-27T14:40:20.485Z
updatedAt: 2022-12-23T20:17:03.808Z
publishedAt: 
firstPublishedAt: 2018-04-27T20:27:49.319Z
contentType: tutorial
productTeam: Financial
author: 245tA425AIeioKAk2eaiwS
slug: configurar-gateway-decidir
locale: es
legacySlug: configurar-gateway-decidir
subcategory: 3tDGibM2tqMyqIyukqmmMw
---

Decidir es un [gateway](/es/tutorial/que-es-un-gateway-de-pagos) de Argentina que permite que su tienda acepte decenas de condiciones de pago.

Para configurar Decidir, siga los pasos abajo:

## Configurar la afiliación de gateway Decidir
1. Acceda al módulo de __Pagos__.
2. Haga clic en __Ajustes__.
3. En la pestaña __Afiliaciones__, haga clic en el botón __+__.
4. Haga clic en el conector __Decidir__.
5. En el campo __Id de la Tienda__, introduzca el código que identifica su tienda.
6. En __Llave secreta__, llene la contraseña registrada en el sistema de Decidir.
7. En el campo __Moneda__, elija la moneda que se utilizará en las transacciones de su tienda. Actualmente, sólo está disponible la opción _Pesos_.
8. En __País__, defina el país de actuación de su tienda. Actualmente, Decidir está disponible sólo en Argentina.
9. En el campo __Captura de seguridad__, elija en cuánto tiempo desea que se realice la captura.
10. En __Antifraude__, informe si su tienda tiene un antifraude configurado.
11. En el campo __Ahora 12__, seleccione si su tienda utiliza el programa de fomento al consumo Ahora 12.
12. En __Plan Gobierno__, la opción _Si_ debe seleccionarse si su tienda utiliza el Ahora 12.
13. En el campo __Change Margin__, defina el margen porcentual en el que se permitirá cambiar el valor del pago.
14. Haga clic en __Guardar__.

Atencion: En los casos de contratación de <strong> CyberSource </strong> antifraude, recomendamos que la tienda se comunique con el equipo de CyberSource para obtener los detalles de configuración de Checkout.

## Configurar la condición de pago
Después de seguir los pasos indicados, Decidir va a estar configurado en su tienda. Por lo tanto, cuando se registre una condición de pago, estará disponible en el campo __Procesar con la afiliación__ (siempre que el medio de pago sea compatible con Decidir). Para saber cómo definir condiciones de pago, acceda a [este artículo del Help](/es/tutorial/condiciones-de-pago).
