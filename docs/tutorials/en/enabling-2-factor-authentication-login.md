...
![EN- LoginAdmin](https://images.ctfassets.net/alneenqid6w5/2POUjooORqKWz4aeLhLtFa/4eaa5520fd10ef3a172cd592756e7698/EN-_LoginAdmin.png)
...
From <strong>November 16, 2017</strong>, 2-Factor Authentication (2FA) becomes <strong>mandatory for all emails</strong> that can access VTEX Admin.<br><br>Without 2FA, you <b>won't be able to log in</b> using just your email and password. This requirement only applies to email and password logins, not to other login methods (Google, Facebook and token).
</div>
...
<div class="alert alert-danger">
We strongly recommend that you <strong>choose the app authentication option</strong>, since, with SMS authentication, you will depend on your network provider. By using an app, you can generate an access key whenever you need it.
</div>
...
If you log in with your email and password on the same computer, the access key will be requested <strong> every three days</strong>. When this happens, check the code in the application.
</div>
...
We cannot guarantee that your network operator will be able to deliver the access key whenever necessary. If the text message does not arrive, try resending the code or <a href="#habilitate-o-2fa-per-application" >enable 2FA through the application</a>.
</div>
