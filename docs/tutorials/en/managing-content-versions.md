...
>⚠️ When accessing <i>Apply to</i>, the option <i>this URL</i> will always appear above <i>this template</i>. This means that if the block has two versions, each one with a different application option, the user will only be able to reactivate the version applied in the template if he deletes the version applied by URL.
>
