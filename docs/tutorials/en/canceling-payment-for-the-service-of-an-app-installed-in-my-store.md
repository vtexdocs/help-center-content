---
title: Canceling payment for the service of an app installed in my store
id: 587GU01BchtMMoQwFtpIcW
status: PUBLISHED
createdAt: 2019-01-30T21:21:20.606Z
updatedAt: 2023-03-29T15:16:06.124Z
publishedAt: 2023-03-29T15:16:06.124Z
firstPublishedAt: 2019-01-30T21:22:07.359Z
contentType: tutorial
productTeam: Others
author: authors_59
slug: canceling-payment-for-the-service-of-an-app-installed-in-my-store
legacySlug: untitled-entry-2019-01-30-at-09-21-20
subcategory: 3e2VLdLao8GGk4sAmMmwmi
---

In order to cancel fees for a service belonging to an app, just __remove the app__ in VTEX admin.
To do this, follow the step below:

1. Access `{store-name}.myvtex.com/admin`.
2. In the side menu, click __Apps__.
3. Click __My Apps__.
4. Click __Settings__ of the desired app.
5. Click __Desinstall__.
6. Clique __Desinstall__ again to save your changes.

After following these steps, you will no longer be charged for the service used by the app.
