...
- **Site Editor** (for stores using the Store Framework): In the VTEX Admin, go to **Storefront > Site Editor**, choose a block that contains an image (e.g. Carousel), and click `Add`.  A modal will open.

- **Headless CMS**: In the VTEX Admin, go to **Storefront > Headless CMS**. Choose one of the created pages, such as a product page. From the available sections, select one to which you can add an image (for example, Hero), and click the Hero image component. A modal will open.
...
1. On the **Media** page, click `Add New`.
...
| Field | Description |
| ---------- | ------------- |
| **View** | Opens the image on a new page. |
