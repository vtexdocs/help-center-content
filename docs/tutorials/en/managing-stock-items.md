...
At VTEX, inventory is the relationship between a store's sales items and the locations where those items are stored. Log in to the VTEX Admin and go to **Catalog > Inventory > Inventory Management** to view the SKUs registered in the [catalog](https://help.vtex.com/en/tracks/catalog-101--5AF0XfnjfWeopIFBgs3LIQ/3rA2tTpIoEXdv2nzC27zxR) and their respective [warehouses](https://help.vtex.com/en/tutorial/warehouse--6oIxvsVDTtGpO7y6zwhGpb).
...
Remember that <b>warehouse</b> is the physical location where the store products are stored, and <b>inventory</b> is the relationship between the warehouses and the SKUs available for sale.
</div>

This article contains information about the **Inventory Management** section in the VTEX Admin. Learn how:
...
A table will display the SKUs and their respective warehouses. The table is organized as follows:
...
* **Columns** identify different aspects of the SKU, and are divided into:
    ...
    * **Last Update**: most recent quantity of SKU units registered on the VTEX platform. <p> You can click on the values in this field to check the **Update log**, with information about the users who updated this field, the dates when the changes occurred, and the quantities before and after the updates</p>
...
In the **Inventory Management** page, you can search for SKUs in the search bar, and also use search filters.

To perform a search, simply type the name or SKU ID and click <i class="fa-solid fa-magnifying-glass"></i> the magnifying glass icon. To use filters, see the available oenions:
