...
When stores sell their products in a marketplace, we have a single-level relationship between seller and marketplace. But VTEX's architecture allows stores to operate both as marketplaces and sellers, expanding the range of possible combinations. 
...
You can configure this feature on [VTEX marketplaces](https://help.vtex.com/en/tutorial/estrategias-de-marketplace-na-vtex--tutorials_402#being-a-vtex-marketplace), [native connectors](https://help.vtex.com/en/tutorial/estrategias-de-marketplace-na-vtex--tutorials_402#integrating-with-a-native-connector-vtex), and [certified partners](https://help.vtex.com/en/tutorial/estrategias-de-marketplace-na-vtex--tutorials_402#integrating-with-a-certified-connector-partner). Even though they support the feature, it is not automatically available for these marketplaces, so you need to configure and adapt the integration for each one. This applies to newly integrated marketplaces and those already using the fulfillment API, which is restricted to two levels. Learn more in the [How to configure it](#how-to-configure-it) section, or in our [article on the Developer Portal](https://developers.vtex.com/vtex-rest-api/docs/multilevel-omnichannel-inventory). 

Multilevel Omnichannel Inventory can cause integration errors when applied to non-VTEX marketplaces. In these scenarios, the order ID will change, for example. We recommend checking with your development team to ensure a seamless migration. Learn more in our [article on the Developer Portal](https://developers.vtex.com/vtex-rest-api/docs/multilevel-omnichannel-inventory). 
...
VTEX Stores cannot change orders made in VTEX marketplaces when they were placed in a Multilevel Omnichannel Inventory scenario. It is necessary to cancel the order and create a new one.

However, VTEX stores can change chain orders made in external marketplaces. For more information, see [Change chain orders in external marketplaces](https://developers.vtex.com/docs/guides/change-orders-multilevel-omnichannel-inventory-external-marketplaces).
...
The marketplace must configure the Multilevel Omnichannel Inventory in the VTEX Admin or via the [Rest API](https://developers.vtex.com/vtex-rest-api/docs/multilevel-omnichannel-inventory). This configuration can be done for integrated sellers and newly integrated ones. 
...
The marketplace should follow the steps below to configure it on already integrated sellers:
   ...
   1. In the Marketplace section in your VTEX Admin, go to the **Sellers** tab.  
   2. Click on **Seller Management**.  
   3. In the row corresponding to the seller whose white-label sellers you want to view, click `Change/View`.
   4. Fill in the **Fulfillment EndPoint** field following the example:  
