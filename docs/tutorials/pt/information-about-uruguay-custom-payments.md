---
title: Informações dos pagamentos customizados do Uruguai
id: 4cXMr3C5rqIkUkMWgCcWeU
status: PUBLISHED
createdAt: 2019-01-24T20:45:36.816Z
updatedAt: 2019-12-31T15:20:18.360Z
publishedAt: 2019-12-31T15:20:18.360Z
firstPublishedAt: 2019-01-24T22:01:06.326Z
contentType: tutorial
productTeam: Financial
author: 245tA425AIeioKAk2eaiwS
slug: informacoes-dos-pagamentos-customizados-do-uruguai
legacySlug: informacoes-dos-pagamentos-customizados-do-uruguai
subcategory: 2Xay1NOZKE2CSqKMwckOm8
---

O Uruguai possui cartões de crédito locais que não possuem integração nativa com a plataforma VTEX, mas que podem ser usados pela sua loja. Para isso, você precisa configurá-los como [pagamentos customizados](/pt/tutorial/configurar-pagamentos-customizados-usando-mercadopago-latam).

Aqui você encontra as informações necessárias para cadastrar com sucesso o cartão OCA. Basta colá-las nos respectivos campos no formulário de cadastro.

## OCA
- __Nome:__ OCA
- __Descrição:__ OCA
- __Meio de Pagamento:__ Mastercard
- __Faixas de Bin{min}-{max},{min}-{max},{bin1},{bin2} (opcional):__ 589892-589892,542991-542991,549530-549530,549564-549564,549571-549571,549576-5495762
- __Código de Pagamento em Adquirente (opcional):__ oca

### Artigos relacionados
- [Que pagamentos customizados estão disponíveis na América Latina?](/pt/faq/que-pagamentos-customizados-estao-disponiveis-na-america-latina)
- [Configurar pagamentos customizados usando MercadoPago (América Latina)](/pt/tutorial/configurar-pagamentos-customizados-usando-mercadopago-latam)
