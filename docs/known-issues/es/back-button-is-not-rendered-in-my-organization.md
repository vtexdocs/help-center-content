---
title: 'El botón "Atrás" no aparece en Mi Organización'
id: xzd2JVKP6jcTN0dGWwISB
status: PUBLISHED
createdAt: 2023-04-25T21:26:35.401Z
updatedAt: 2023-05-12T20:42:29.822Z
publishedAt: 2023-05-12T20:42:29.822Z
firstPublishedAt: 2023-04-25T21:26:36.170Z
contentType: knownIssue
productTeam: B2B
author: 2mXZkbi0oi061KicTExNjo
tag: B2B
slug: el-boton-atras-no-aparece-en-mi-organizacion
kiStatus: Fixed
internalReference: 797003
---

## Sumario

<div class="alert alert-info">
  <p>Este problema conocido ha sido traducido automáticamente del inglés.</p>
</div>


El botón "Atrás" no se muestra en Mi Organización en los móviles, lo que impide al comprador acceder a otras opciones desde Mi Cuenta.


##

## Simulación



- Inicie sesión en el sitio web desde un móvil;
- Acceda a Mi cuenta;
- Seleccione Mi Organización; no hay botón "Atrás".



## Workaround


Haga clic para volver desde el navegador.




