---
title: 'In fullcleanup admin the Reindex Database button may not work on the first try'
id: 4spOMbnKZPCcnQghKTP6Zb
status: PUBLISHED
createdAt: 2023-04-14T17:39:56.758Z
updatedAt: 2023-04-14T17:39:57.290Z
publishedAt: 2023-04-14T17:39:57.290Z
firstPublishedAt: 2023-04-14T17:39:57.290Z
contentType: knownIssue
productTeam: Catalog
author: 2mXZkbi0oi061KicTExNjo
tag: Catalog
slug: in-fullcleanup-admin-the-reindex-database-button-may-not-work-on-the-first-try
locale: en
kiStatus: Backlog
internalReference: 790828
---

## Summary


In the Fullcleanup admin, there is a button to reindex the products of the account. It is expected that this button will perform the action on the first attempt, however, for the action to be performed it may be necessary to click the button several times.


##

## Simulation



1. Open Fullcleanup admin - `/Admin/Site/FullCleanUp.aspx  `
2. Click the button **Reindex Database**
3. Nothing may occur


##

## Workaround


Try to click on the bottom more times




