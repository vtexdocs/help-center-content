...
> ⚠️ If the order remains in the status `Ready for handling`, request more information by opening a ticket with [VTEX Support](/en/support).
...
> ⚠️ If the order remains in the status `Ready for handling` after the manual update attempt, request more information by opening a ticket with [VTEX Support](/pt/support).
