...
Through the [Message Center](/en/docs/tutorials/understanding-the-message-center), VTEX allows merchants to customize email templates to be sent to users. These emails can be used to convey the following information:
...
On VTEX, you can use the native mail server or [configure an external SMTP server](/en/docs/tutorials/customizing-the-vtex-smtp). If you are using an external SMTP server, see the scenarios below.
...
Check if the external SMTP server is configured according to the [SPF system](/en/docs/tutorials/best-practices-on-spf), which prevents unauthorized servers from sending emails on behalf of a specific domain. You can configure [SPF](https://developers.vtex.com/docs/guides/setting-up-the-spf) and [DKIM](https://developers.vtex.com/docs/guides/setting-up-dkim-for-transactional-emails) (DomainKeys Identified Mail) on the server if necessary.
...
![copia_email_en](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/en/troubleshooting/data-access-and-security/i-cant-receive-emails-from-vtex_1.png)
