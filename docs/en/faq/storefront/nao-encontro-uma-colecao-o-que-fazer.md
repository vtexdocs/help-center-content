---
title: "I can't find a collection. What do I do?"
excerpt: "CMS lists only the 20 most recently updated collections. Search by name to open any collection that is not in that list."
id: 4jqVY2ilvOQYwmuOY0sAyk
status: PUBLISHED
createdAt: 2018-01-24T18:25:15.475Z
updatedAt: 2020-12-11T16:37:16.306Z
publishedAt: 2020-12-11T16:37:16.306Z
firstPublishedAt: 2018-01-24T18:57:50.665Z
contentType: frequentlyAskedQuestion
productTeam: Storefront
author: authors_24
slugEN: nao-encontro-uma-colecao-o-que-fazer
locale: en
legacySlug: nao-encontro-uma-colecao-o-que-fazer
---

> ⚠️ There are two ways to configure collections, through the CMS or the Collection module (Beta). This article is about how to [configure collections through the CMS](/en/docs/tutorials/adding-collections-cms).

By default, the CMS module only displays the __last 20 collections__ updated.

So if your store has more than 20 collections, it's normal for some of them not to appear in the list shown in the CMS.

![findCollection1](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/en/faq/storefront/nao-encontro-uma-colecao-o-que-fazer_1.png)

But you can fetch any collection using the __search field__ (Find), including those that are not displayed by default.

![findCollection2](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/en/faq/storefront/nao-encontro-uma-colecao-o-que-fazer_2.png)
