---
title: "What to do when the product's NSU is duplicated in transactions performed by Rede?"
excerpt: "Rede issues the NSU, not VTEX. If two transactions share an NSU, reconcile by TID and contact Rede to fix the duplicate."
id: 6GCSJnX9pnCk8jVOQ4cDK1
status: PUBLISHED
createdAt: 2019-02-27T20:58:12.700Z
updatedAt: 2019-12-31T14:24:44.752Z
publishedAt: 2019-12-31T14:24:44.752Z
firstPublishedAt: 2019-02-27T21:00:00.706Z
contentType: frequentlyAskedQuestion
productTeam: Payments
author: 10Jj3Ca0hIsTVp4BEeef6Q
slugEN: what-to-do-when-the-products-nsu-is-duplicated-in-transactions-performed-by-rede
locale: en
legacySlug: 
---

Sometimes, the [NSU](/en/docs/tutorials/how-do-i-find-the-order-nsu-and-tid) (Single Sequential Number) may be duplicated by the acquirer Rede. Errors like this can disrupt the conciliation process.

However, in circumstances where the NSU is the same for two different customers, an alternative to verify the transaction is to process the payment using the [TID](/en/docs/tutorials/how-to-view-the-orders-details) numbers, which are different from each other.

> The NSU is not created by VTEX, but by the acquirer responsible for the transaction. Therefore, it's important to contact Rede's e-commerce team to solve the problem.
