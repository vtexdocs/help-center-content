---
title: 'When trying to approve payment, Orders Management returns an "authorize-fulfillment" error. What should I do?'
excerpt: "Authorize-fulfillment fails when the SKU reservation was released and stock is gone. Confirm inventory in View Interactions before retrying approval."
...
In such a case, when you try to approve the payment in **Orders**, you will see the following error:

![error authorize fulfillment](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/en/faq/orders/when-trying-to-approve-payment-orders-managment-returns-authorize-fulfillment-error-what-to-do_1.png)
...
If there is an error such as "The requested reservation *00-abc* could not be guaranteed for the *00-xyz* order", it is because one or more of the SKUs included in the cart don’t have available inventory, which prevents the purchase from being approved. In this case, correcting the inventory should allow the order to move on.
