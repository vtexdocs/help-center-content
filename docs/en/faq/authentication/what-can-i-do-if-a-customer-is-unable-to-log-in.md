...
excerpt: "After three failed login attempts, VTEX ID blocks the email for one hour automatically. Missing password emails can also mean the address is on the AWS SES suppression list."
...
But since VTEX uses the same platform for all its clients, the blacklist operates for the same dispatch quota and rejection rate. An email address can remain on the blacklist for up to 14 days.
