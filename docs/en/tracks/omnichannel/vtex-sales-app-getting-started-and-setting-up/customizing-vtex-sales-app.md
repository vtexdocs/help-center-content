...
* [Enable cart transfer and capture between devices](https://developers.vtex.com/vtex-rest-api/docs/enable-cart-transfer-between-devices): allow a purchase that was started in one device to be completed in any other device present in the purchase flow without having to add the products to the cart again. For more information, go to [this article](/en/docs/tracks/cart-transfer-and-capture).
