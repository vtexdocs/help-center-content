---
title: 'Adding Rounding Rules'
id: 1QDJfW5ALy4W8pRWg2AWt1
status: PUBLISHED
createdAt: 2019-07-04T13:54:05.527Z
updatedAt: 2023-03-29T20:33:28.498Z
publishedAt: 2023-03-29T20:33:28.498Z
firstPublishedAt: 2019-07-17T19:47:09.101Z
contentType: trackArticle
productTeam: Marketing & Merchandising
slugEN: adding-rounding-rules
locale: en
trackId: 6f8pwCns3PJHqMvQSugNfP
trackSlugEN: prices-101
order: 13
---

There are two of adding a rounding rule:

- through Admin
- by API REST

Follow the steps below to add a rounding rule to your store:

1. In the VTEX Admin, go to *Product > Price Rouding Rules*, or type *Price Rouding Rules* in the search bar at the top of the page.
3. Click on the **New Rule** button.
4. In the first step, **What is the price range to which this rule will apply?**, set a **From** - **Up To** price range to which the psychological pricing applies. 
5. In the second step, **How many digits will be rounded?**, select the number of digits to apply psychological pricing to from the dropdown menu.
6. In the third step, **Which rounding method will be applied?**, select the [desired method](/en/docs/tracks/rounding-rules-concept-definition).
7. Click on **Test your prices** and insert a price to check whether the newly created rule works as intended.
8. Lastly, click on **Save**.

