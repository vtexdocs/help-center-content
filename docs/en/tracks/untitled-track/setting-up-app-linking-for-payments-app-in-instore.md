...
![Payment app](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/en/tracks/untitled-track/setting-up-app-linking-for-payments-app-in-instore_1.png)
...
- `acquirerId`: string - affiliation id of the acquirer which is registered in [VTEX Payments](/en/tutorial/setting-up-app-linking-for-payments-app-in-instore#setting-up-instore-connector-in-vtex-pci-gateway). Ex: `<stone_code>`, `<sitef_storeId>`.
...
- `firstDigits`: string - card BIN / IIN (first six digits) - optional
- `lastDigits`: string - card last four digits - optional
- `cardToken`: string - token of the card used on this purchase
