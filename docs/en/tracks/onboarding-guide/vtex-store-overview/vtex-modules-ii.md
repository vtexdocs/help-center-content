...
| **Payment Provider Protocol** | Protocol used by payment or anti-fraud providers to transmit payment data. Learn more about how providers integrate with VTEX in [Payment Provider Protocol](/en/docs/tutorials/payment-provider-protocol). |
...
1. Access the [List of Payment Providers by Country](/en/docs/tutorials/list-of-payment-providers-by-country).
...
8. If the chosen payment method allows you to set up payments in installments, select the **In installments** option under **Paid in full or in installments?** and enter the desired information. Learn more in [Configuring payment terms](/en/docs/tutorials/how-to-configure-payment-conditions).
...
- **Notes payable**: Payment commitment between the merchant and the customer for the purchase amount. Manual approval by the merchant is required to approve the purchase transaction. Learn more in [Setting up payments with Notes Payable](/en/docs/tutorials/setting-up-payments-with-notes-payable).

- **Store cards (private label)**: Credit or debit cards issued through a partnership between a company and a card label, such as Visa or Mastercard. These cards can only be used for purchases at establishments that accept the card issuer's label. Learn more in [Setting up Private Label payments](/en/docs/tutorials/setting-up-private-label-payments).

- **Store cards (co-branded)**: Credit or debit cards that are branded with a company's own name, issued exclusively for purchases made at the establishments of the card-issuing company. Learn more in [Setting up payments with Store Card (cobranded)](/en/docs/tutorials/setting-up-payments-with-store-card-cobranded).
...
To view the anti-fraud providers that are currently integrated with VTEX, enter the name of the provider into the [Help Center](/en/) search bar or access the VTEX Admin, as detailed in the article [Configuring the anti-fraud](/en/docs/tutorials/how-to-configure-the-anti-fraud).
...
However, for a customer to make a purchase and place an order, the store must have registered and priced products, offer [payment methods](/en/docs/tutorials/difference-between-payment-methods-and-payment-conditions), and ensure that the order can be shipped. To do this, the following modules must be configured:
...
In the VTEX Admin, under **Orders > [All Orders](/en/docs/tutorials/all-orders)**, you can perform the following actions:

- View a list of all store orders.
- Access order details on the [details page](/en/docs/tutorials/order-details-page) by clicking the order.

On the order details page, you can also track the progress of the [order flow](/en/docs/tutorials/order-flow-and-status). The image below illustrates an example of this flow when the store acts as both a marketplace and a seller:
...
VTEX natively offers the option of sending [transactional emails](/en/docs/tutorials/understanding-the-message-center), enabling stores to automatically inform customers of order updates. In essence, as the order progresses through the processing flow, customers can receive notification emails. These emails keep them updated on key stages, such as payment approval and order shipping.

The platform offers a series of [transactional email templates](/en/docs/tutorials/order-transactional-email-templates), but customizing these communications is also possible. The list below describes the main aspects of these optional settings, which are independent of each other:

- **Alias**: Alternative email address automatically generated by the VTEX platform to [mask](/en/docs/tutorials/general-configurations-on-the-oms) the customer's email in store transactional emails. This [type of conversation mask](/en/docs/tutorials/general-configurations-on-the-oms) provides enhanced protection for customer data, as the alternative email is designed to be unreadable by external systems.

- **Availability - Notify Me**: Configuration of the [Notify Me](/en/docs/tutorials/setting-up-the-notify-me-option) button on product pages visible on the store website. This button appears when a product is out of stock, allowing customers to enter their email to receive a notification when the product is available for purchase again.

#### My Account feature

[My Account](/en/docs/tutorials/how-my-account-works) is the solution that gives customers autonomy to check their orders and update their personal information. It's a unique environment for each customer and can be accessed with a username and password.
...
Learn more in [Orders General Configuration](/en/docs/tutorials/general-configurations-on-the-oms).

## Checkout

Checkout is the last stage in the online shopping process, playing a significant role in achieving a good sales conversion rate. On the VTEX platform, the [Checkout](/en/docs/tutorials/checkout-vtex-overview) module is crucial for:
...
Learn more in [Optimization of shipping options at Checkout](/en/docs/tutorials/optimization-of-shipping-options-at-checkout).
...
Learn more in [Setting up Cart Abandonment](/en/docs/tutorials/setting-up-abandoned-carts).
...
> ⚠️ Only users with the necessary [roles](/en/docs/tutorials/roles) and permissions within the VTEX Admin can make these changes.

Learn more in [Change the price of an item in the shopping cart](/en/docs/tutorials/change-the-price-of-an-item-in-the-shopping-cart).
...
Learn more in [How to configure minimum order value and maximum quantity of items per SKU](/en/docs/tutorials/how-to-configure-minimum-order-value-and-maximum-quantity-of-items-per-sku).
...
Learn more in [Setting up geolocation at checkout](/en/docs/tutorials/geolocation-at-checkout).
