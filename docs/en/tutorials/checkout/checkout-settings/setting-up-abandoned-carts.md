...
In this step, you must create a [trigger in Master Data](/en/docs/tutorials/creating-trigger-in-master-data), which involves completing some values to determine the trigger and defining some filters:
...
- __Receives newsletter?:__ Ensures compliance with the [rights of data subjects](/en/docs/tutorials/data-subject-rights), since only customers subscribed to the newsletter will receive abandoned cart emails. Subscription to your store's newsletter can be managed at checkout or in the [My Account](/en/tutorial/como-funciona-a-minha-conta--2BQ3GiqhqGJTXsWVuio3Xh#perfil) section of your store. The trigger will be enabled if the value is true.
