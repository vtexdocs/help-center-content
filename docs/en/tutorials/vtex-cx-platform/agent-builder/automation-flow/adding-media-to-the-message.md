...
- If the communication channel is WhatsApp, it has its own rules, as shown in their [documentation](https://developers.facebook.com/docs/whatsapp/cloud-api/reference/media#supported-media-types). We can see that videos and audio have a maximum size limit of 16MB (if you upload a 20MB video to the VTEX CX Platform, it will be saved on the platform, but when attempting to send it, WhatsApp won't execute the delivery). Documents have a maximum limit of 100MB each (however, the VTEX CX Platform supports up to 32MB).
