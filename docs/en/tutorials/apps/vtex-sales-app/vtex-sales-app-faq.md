...
An additional benefit of completing orders with a shared cart is that they go through the [anti-fraud solution](/en/docs/tutorials/financial-agents-in-the-payment-flow#anti-fraud) flow, which isn't currently supported in **VTEX Sales App** payment **settings**.
