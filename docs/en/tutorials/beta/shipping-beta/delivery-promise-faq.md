...
* Pricing.
* A [shipping strategy](/en/docs/tutorials/shipping-strategy) that enables the order to be shipped to the address provided by the customer.
* Product count in the [inventory](/en/docs/tutorials/managing-stock-items), unlimited inventory, or the option to show when the product is out of stock.
* [CMS](/en/docs/tracks/what-is-the-cms) templates of the store storefront.
...
* Keep the product [inventory](/en/docs/tutorials/managing-stock-items) with stock available for sale. Remember that [reserved](/en/docs/tutorials/how-does-reservation-work) items aren't available for sale.
* Enable the [unlimited inventory](/en/docs/tutorials/managing-stock-items) configuration for the product.
...
* Define [comprehensive sellers](/en/docs/tutorials/comprehensive-seller), so products from these sellers can be displayed to customers who don't provide their location.
* Set a default postal code or [geocoordinates](/en/docs/tutorials/registering-geolocation) for customers who don't provide their location. We suggest setting this near a [warehouse](/en/docs/tutorials/warehouse) that stores a large quantity of products, or near the region with the highest sales volume.

## 6. Where do I configure which stores cover specific postal codes?

Even for those using **Delivery Promise (Beta)**, you still configure this using the [shipping rate template](/en/docs/tutorials/shipping-rate-template) of the sellers' [shipping policies](/en/docs/tutorials/shipping-policy).
...
Using Delivery Promise (Beta) is optional. If you don't want to request the customer's location, you can continue using the current customer navigation solution without a defined location and choose whether to link it to [comprehensive sellers](/en/docs/tutorials/comprehensive-seller). However, we recommend using **Delivery Promise (Beta)** to provide the best shopping experience on the product listing page.
