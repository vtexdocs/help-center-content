...
Custom checkout fields let you collect additional information during purchase in the B2B Buyer Portal, such as cost center, internal order number (PO), and other control data. These values are associated with the order, helping the company standardize data, enforce internal rules, and support audits.
...
With custom fields, your company can:
...
There are two main profiles for custom fields:
...
When creating a custom field, the administrator must choose one of the following types.
