...
Lead time is an optional configuration for shipping time at SKU level. This means that you can set a shipping period for a [warehouse](/en/docs/tutorials/warehouse) SKU, and this time will be added to the order's total shipping time.

Lead time configuration is done within [inventory management](/en/docs/tutorials/managing-stock-items) as it relates to items in a warehouse that are available for sale.
...
- [Warehouse time:](/en/docs/tutorials/managing-warehouses) Time the SKU takes to leave a warehouse and arrive at a [loading dock](/en/docs/tutorials/loading-dock). This configuration is required, but you can set the time to zero.
- [Loading dock time:](/en/docs/tutorials/managing-loading-docks) Time the SKU takes to leave the loading dock, the point between the warehouse and the [carrier](/en/docs/tutorials/carriers-on-vtex), and be shipped to the customer. This configuration is required, but you can set the time to zero.
- [Shipping policy:](/en/docs/tutorials/shipping-policy) Gathers the rules and conditions displayed to customers at checkout, including:
    - [Shipping rates](/en/docs/tutorials/shipping-rate-template)
...
Lead time is set to zero, and the limit is 365 days. By default, the calculation is made in business days, considering public holidays and other [shipping policy](/en/docs/tutorials/shipping-policy) conditions, but you can change the [calculation to calendar days](#configuring-lead-time-calculation-in-calendar-days-optional).
...
2. Find the shipping policy you want. You can use the search bar and filters.
    > To create a new shipping policy, read the [Shipping Policy](/en/docs/tutorials/shipping-policy) article.
