...
On the VTEX platform, warehouses are physical storage spaces for goods sold by the ecommerce business. The shipping logistics process starts at the warehouse. Items are stored, separated, sent to [loading docks](/en/docs/tutorials/managing-loading-docks) and then delivered to customers by carriers.
...
> ℹ️ The carrier that will deliver a certain order is defined by the [shipping policy](/en/docs/tutorials/shipping-policy). For a carrier to be selected to deliver the order, its shipping policy must meet the conditions required for the delivery of the order. The shipping policy is linked to the warehouse by the Loading dock.

On the VTEX platform, you can add the name of the warehouse, add items to the warehouse, [configure the loading docks](/en/docs/tutorials/managing-loading-docks) where products will be sent to, configure how long it takes for the order to be prepared before it is sent to the loading dock and the amount charged for its handling. 
...
* To learn how to create, modify, and monitor warehouses, see the article [Warehouse management](/en/docs/tutorials/managing-warehouses). 
