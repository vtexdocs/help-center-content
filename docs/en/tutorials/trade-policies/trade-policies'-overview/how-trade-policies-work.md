...
> ⚠️ If you have many sub-accounts, or physical store operations that offer in-store pickup or ship from store, creating multiple trade policies is not the best solution. We suggest creating new [store names](/en/tutorial/o-que-e-store-name) for each store within an [account](/en/tutorial/o-que-e-account-name).
