...
* **Usage**: Displays the number of times the coupon can be used and the number of remaining uses, if you have set a usage limit.
    ...
    If it is a coupon batch, this section displays the list of codes from the batch and allows you to search for a specific code. If you click a code, you will see the list of orders where the code has been used, and you can search for an order number. Clicking on the order number will then take you to the order details page in __Orders management__.
...
* **UTM source:** Allows linking the coupon to a specific `utm_source`. The `utm_source` represents the source tracking, i.e. which site, ad or post the user came from. Example: you can set it so that whenever a user reaches your store by clicking on a Facebook ad, the URL will contain the parameter {f1}utm_source=fb{/f1} and the coupon will be applied. For more information, please read [What are utm_source, utm_campaign and utm_medium](/en/docs/tutorials/what-are-utm-source-utm-campaign-and-utm-medium).
...
* <i class="fas fa-toggle-on"></i> **Generating coupons in a batch:** Keeping this option deactivated will create a single coupon. By activating this option, you will create a [coupon batch](/en/docs/tutorials/coupons-beta#coupon-batches) and the following resources will become available. 
    * **Number of coupons:** Allows you to define the number of codes generated in a coupon batch. The maximum number is 1000.
...
* <i class="fas fa-toggle-on"></i> **Limit coupon usage in my store:** When this option is enabled, you must specify the number of times the coupon can be used, either by using the buttons <i class="fas fa-minus"></i> and <i class="fas fa-plus"></i> or by typing the desired quantity. The minimum number is 1 and there is no maximum number.
  ...
  > ⚠️ In case an order with a limited usage coupon applied is canceled, the limit will be recalculated. This means that orders with the **Canceled** status will not be counted toward the coupon usage limit.
...
To enable a coupon, you must associate it with a promotion. The coupon can be associated with all types of promotions, except [Campaign promotions](/en/docs/tutorials/campaign-promotion). To do this, follow the steps below:
...
4. In the **What are the conditions for the promotion to be valid?** section, fill in the **utm_source** or **utm_campaign** fields with the same values as the created coupon.
5. Click `Save`.
...
You can allow your customers to use multiple coupons in a purchase. This feature is only available via API, with a limit of 20 coupons per purchase. Use one of the following endpoints to enable it.

Both endpoints fill in the `marketingData` field of the response body and work the same way for activation. Follow the steps below to enable the feature:
...
2. [Customize](https://developers.vtex.com/docs/guides/checkout-customization-guide#customizing-checkout-ui-admin-vtex) your storefront to allow multiple coupons.
   - If your store uses Checkout v5, use `checkout5-custom.js` and `checkout5-custom.css`.
