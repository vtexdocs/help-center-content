...
We define as a __seller__ the store responsible for delivering an order. The seller is responsible for the [fulfillment](/en/docs/tutorials/fulfillment-logistics-vtex) process, ie it's the store that must comply with the obligation to deliver the product to the customer.
...
A __seller__ can sell its catalog in a third-party store, be it VTEX or not; this seller can also sell its products in its own store, so it acts at the same time as a __seller__ and a __marketplace__, which is called a [white label seller](/en/docs/tutorials/white-label-seller).
...
- __White label seller:__ The seller remains hidden in the consumer's buying experience, being [automatically selected](/en/docs/tutorials/white-label-sellers-selection) by the VTEX platform to ship the order.  
