...
![EN Credit Card Flow](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/en/tutorials/payments/payment-methods-and-conditions/cards-benefits-and-gift-cards/credit-card-basic-payment-flow_1.svg)

1. On the Checkout screen, the customer chooses the credit card __Payment Method__ to pay for their purchase.
2. Payment information is passed to the __Payment Gateway__. The [gateway](/en/docs/tutorials/financial-agents-in-the-payment-flow#gateway) is the player responsible for ensuring that this payment is successful.
3. With the information in hand, the payment gateway passes on the required information to the __Acquirer__. The [acquirers](/en/docs/tutorials/financial-agents-in-the-payment-flow#acquirer) are responsible for settlements of financial transactions with retailers.
4. In the possession of this information, the acquirer sends the payment data to the __Card Brand__. The [card brand](/en/docs/tutorials/financial-agents-in-the-payment-flow#credit-card-brand) determines some business rules for payments, such as the number of installments. In addition, it connects the acquirer system with the issuing banks.
