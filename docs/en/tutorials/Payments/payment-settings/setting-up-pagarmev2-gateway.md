...
> ⚠️ If you want to configure the V3 version of Pagarme in your store, go to [Setting up payments with PagarMeV3](https://help.vtex.com/en/tutorial/setting-up-payments-with-pagarmev3--5PMavdamrVijwIrHas9ndE).
...
<li>**Payment capture:**: select one of the following options:<br><br>
    <ul>
    	...
    	<li>**Automatic capture immediately after payment authorization**: capture is automatically performed right after payment authorization, even if the transaction includes an anti-fraud analysis.</li>
   		...
   		<li>**Deactivated: Not automatically captured**: capture is only performed after the order has been invoiced. Please note that the payment will not be processed if invoicing is delayed and exceeds the capture period agreed with the payment provider.</li>
    </ul>
  </li>
</ul>
...
<ul>
  <li>**Responsibility for payment fees**: Choose from Marketplace, Markeplace + Sellers or Sellers.</li>
  <li>**Responsibility for chargebacks**: Choose from Marketplace, Markeplace + Sellers or Sellers.</li>
</ul>
7. Click on __Save__.
