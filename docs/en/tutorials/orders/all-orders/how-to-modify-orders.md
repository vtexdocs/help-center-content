...
The [Order modifications](/en/docs/tutorials/how-order-modification-works) feature allows you to edit an order in different ways, either due to customer requests, product unavailability, or other reasons.
...
In the [order flow](/en/docs/tutorials/order-flow-and-status), `Handling shipping` is the only status valid for **Order modifications** in the VTEX Admin. Read the [How Order modifications works](/en/docs/tutorials/how-order-modification-works) article to learn more about API modifications.

### License Manager permission

For a user to be able to modify orders, their [role](/en/docs/tutorials/roles) must be associated with at least one of the following [Licence Manager resources](/en/docs/tutorials/license-manager-resources):
...
- **Price change:** [Change the order total amount](/en/docs/tutorials/changing-the-total-cost-of-the-order).

All these actions are done on the [order details](/en/docs/tutorials/order-details-page) page, which you can access in two ways, as described below.

### Viewing orders on the All orders page

In the VTEX Admin, go to **Orders >** [All Orders](/en/docs/tutorials/all-orders) and click the desired order, or type **All Orders** in the search bar at the top of the page. Then click the order you want to view. If necessary, you can use [filters](/en/docs/tutorials/filtering-all-orders) to narrow your search.
...
> ℹ️ You can make a single modification to the order or combine several at once. There is no limit to how many times an order can be modified. Read the [How Order modifications works](/en/docs/tutorials/how-order-modification-works) article to learn more.
...
- **Insufficient inventory:** The number of items added or replaced in the modification may be greater than the number of items available for sale in your [inventory](/en/docs/tutorials/managing-stock-items).
