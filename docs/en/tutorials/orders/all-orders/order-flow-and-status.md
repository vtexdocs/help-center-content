...
> ℹ️ Learn how to track an order flow in the article [Order tracking](/en/tutorial/acompanhamento-de-pedidos--et0Ei7F3bjcrEmVAR2kKS).
...
> ⚠️ For incomplete orders, check out the article [How incomplete orders work](/en/tutorial/entendendo-os-pedidos-incompletos--tutorials_294) to understand their flow and cancellation.
...
> ❗ Note that when the cancellation occurs before the store starts handling the order, the items have the [reservation cancelled](/en/tutorial/how-does-reservation-work--tutorials_92) in inventory.  However, if the cancellation is made after this stage, the [items will already be reserved](/en/tutorial/como-a-reserva-funciona--tutorials_92#acknowledged-reservation) in the inventory. So the store must update the number of items in stock. This will make the order SKUs available for sale again.
