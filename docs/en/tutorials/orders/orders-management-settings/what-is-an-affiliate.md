...
[Configuring an affiliate](/en/tutorial/como-configurar-afiliado) will only be necessary when the store acts as a seller in a marketplace.

An affiliate functions differently from [sales channels](/en/docs/tutorials/how-trade-policies-work), since every marketplace must correspond to a unique affiliate code for the seller. In other words, the seller creates an affiliate code to identify each marketplace. Even if the same sales channel in being used, the affiliates must be different. If the integration is with a [native connector](/en/docs/tutorials/marketplace-strategies-at-vtex#integrating-with-a-native-connector-vtex), the [affiliate](/en/tutorial/como-configurar-afiliado/) will be created automatically.

Besides identifying new orders, affiliates also act as webhooks that will notify the external service about changes on SKU's informations, inventory or pricing. Learn more in [Integration Guide - Consuming catalog information for use in an External Service](/en/docs/tutorials/integration-guide-consuming-catalog-information-for-use-in-an-external-service).
