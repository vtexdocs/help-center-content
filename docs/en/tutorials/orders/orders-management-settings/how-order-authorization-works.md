...
When the price defined by the seller is different from the price offered by the marketplace, orders may not be processed correctly. On VTEX, the **Order Authorization** flow allows you to control and automatically approve these orders with the [Price Divergence rule](/en/docs/tutorials/price-divergence-rule). 
