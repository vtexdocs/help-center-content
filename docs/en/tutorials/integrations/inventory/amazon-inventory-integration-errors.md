...
This message indicates a failure to export the SKU to the Amazon catalog, probably because the mapping template was not filled in correctly for that SKU. This makes it impossible to integrate inventory with Amazon. First, you must re-export the category to which the SKU belongs. To do this, please refer to the article [Sending products to Amazon](/en/tracks/configurar-integracao-com-a-amazon--6sgd4Pagy3wNsWKBvmIFrP/5xklf2wSdeztQh4iy5kJvD). Then, [update your inventory](/en/docs/tutorials/updating-the-quantity-of-items-in-inventory). This will automatically be reflected in Amazon, therefore, no manual reprocessing of the error is required.
...
](/en/docs/tutorials/minimum-stock-control-for-integrations)
- [Setting up the Alert Low Inventory
](/en/docs/tutorials/setting-up-the-critical-stock-alert)
