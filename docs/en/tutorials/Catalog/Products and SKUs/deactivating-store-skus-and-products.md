---
title: 'Deactivating store SKUs and products'
id: 4oviRZMwCkSEcugEyEmEwo
status: PUBLISHED
createdAt: 2017-10-31T03:53:15.045Z
updatedAt: 2023-03-29T19:32:29.834Z
publishedAt: 2023-03-29T19:32:29.834Z
firstPublishedAt: 2017-10-31T04:03:55.888Z
contentType: tutorial
productTeam: Marketing & Merchandising
author: authors_24
slugEN: deactivating-store-skus-and-products
locale: en
legacySlug: deactivating-store-skus-and-products
subcategoryId: pwxWmUu7T222QyuGogs68
---

A product is only deactivated when all its SKUs are inactive.

To deactivate the SKU of a product, proceed as follows:

1. In the VTEX Admin, go to __Catalog__, or type __Catalog__ in the search bar at the top of the page.
2. Click on __All products__
3. Search for the product you want in the list.
4. Click on the SKU which you want to deactivate.
5. Deselect the option __Activate the SKU if possible__.

![deactivateSKU](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/en/tutorials/Catalog/Products%20and%20SKUs/deactivating-store-skus-and-products_1.png)
