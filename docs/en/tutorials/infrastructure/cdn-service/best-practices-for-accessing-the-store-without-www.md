...
* [How DNS configuration works on VTEX](/en/docs/tutorials/how-dns-configuration-works-on-vtex)
...
VTEX recommends always using the `www` or another subdomain when[ configuring the store's main domain](/en/docs/tutorials/configuring-the-store-domain), to combine the use of CNAMEs and other settings. In this scenario, we recommend using redirects to access the address without `www` or other subdomains. Read the [Redirection from other addresses](/en/docs/tutorials/redirect-from-other-addresses) article to learn how to configure redirects.
...
> ⚠️ When the DNS Flattening solution is used, it will have the same behavior as a[ reverse-proxy](/en/docs/tutorials/insert-reverse-proxy-in-front-of-vtex-services). In this case, a TXT verification entry must be configured.
