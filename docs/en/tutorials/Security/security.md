---
title: 'Security'
id: security
status: PUBLISHED
createdAt: 2024-11-05T19:06:37.704Z
updatedAt: 2024-11-05T23:41:09.286Z
contentType: tutorial
productTeam: Security
slugEN: security
locale: en
---

## Store security

- [Best practices against virtual attacks](https://help.vtex.com/en/docs/tutorials/best-practices-against-virtual-attacks)
- [Erasing customer data](https://help.vtex.com/en/docs/tutorials/erasing-customer-data)
- [Enabling two-factor authentication login](https://help.vtex.com/en/docs/tutorials/enabling-2-factor-authentication-login)
- [Enable HTTPS throughout the site](https://help.vtex.com/en/docs/tutorials/enable-https-throughout-the-site)
- [Data subject rights](https://help.vtex.com/en/docs/tutorials/data-subject-rights)
- [Best practices for using application keys](https://help.vtex.com/en/docs/tutorials/best-practices-application-keys)
- [Best practices to avoid scams using the VTEX brand](https://help.vtex.com/en/docs/tutorials/best-practices-to-avoid-scams-using-the-vtex-brand)
- [Synonym list](https://help.vtex.com/en/docs/tutorials/synonym-list)
- [Managing HTTP headers](https://help.vtex.com/en/docs/tutorials/managing-http-headers)

## Platform security resources

- [Security certificate (SSL)](https://help.vtex.com/en/docs/tutorials/security-certificate-ssl)
- [SmartCheckout Security](https://help.vtex.com/en/docs/tutorials/smartcheckout-security)
- [Audit](https://help.vtex.com/en/docs/tutorials/audit)
- [Events available in Audit](https://help.vtex.com/en/docs/tutorials/events-available-in-audit)

## Information security compliance

- [Risk Assessment](https://help.vtex.com/en/docs/tutorials/risk-assessment)
- [Penetration tests and vulnerability notifications](https://help.vtex.com/en/docs/tutorials/penetration-testing-and-vulnerability-notice)

## VTEX Shield

- [VTEX Shield](https://help.vtex.com/en/docs/tutorials/vtex-shield)
- [Security Monitor](https://help.vtex.com/en/docs/tutorials/security-monitor)
- [Penetration tests and vulnerability notifications](https://help.vtex.com/en/docs/tutorials/penetration-testing-and-vulnerability-notice)
- [Web Application Firewall (WAF)](https://help.vtex.com/en/docs/tutorials/web-application-firewall-waf)

