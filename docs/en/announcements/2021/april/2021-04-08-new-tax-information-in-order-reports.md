...
The **Taxes** column in the [order report](/en/docs/tutorials/order-report) has been modified to make it easier for retailers to view the total amount of taxes applied to each order.

## What has changed?

Previously, the **Taxes** column listed only taxes [added at **Checkout**](/en/docs/tutorials/inserting-taxes-in-the-cart) (values identified as `Tax`). The report did not account for custom taxes[ created in the Admin's **Promotions & Taxes** module](/en/docs/tutorials/creating-surchargestaxes), which means it didn’t include values specified as `CustomTax`.
...
Learn how to export orders in our tutorial [Export orders in Orders management](/en/docs/tutorials/exporting-orders-in-orders-module) and learn more about the report structure in our article [Order report](/en/docs/tutorials/order-report).
