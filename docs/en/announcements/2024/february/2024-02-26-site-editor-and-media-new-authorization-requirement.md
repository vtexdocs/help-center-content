...
> ⚠️ To manage users and their roles, you need the `Save access profile` resource from the <bold>License Manager</bold> product associated with your user role. For example, the <a href=”/en/tutorial/predefined-roles--jGDurZKJHvHJS13LnO7Dy#user-administrator-restricted”> User Administrator - RESTRICTED</a> is a predefined role that has the `Save access profile` resource associated with it.
