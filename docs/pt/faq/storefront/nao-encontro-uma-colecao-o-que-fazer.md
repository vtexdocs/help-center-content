---
title: 'Não encontro uma coleção. O que fazer?'
excerpt: "O CMS lista só as 20 coleções atualizadas mais recentemente. Busque pelo nome para abrir qualquer coleção que não esteja nessa lista."
id: 4jqVY2ilvOQYwmuOY0sAyk
status: PUBLISHED
createdAt: 2018-01-24T18:25:15.475Z
updatedAt: 2020-12-11T16:37:16.306Z
publishedAt: 2020-12-11T16:37:16.306Z
firstPublishedAt: 2018-01-24T18:57:50.665Z
contentType: frequentlyAskedQuestion
productTeam: Storefront
author: authors_24
slugEN: nao-encontro-uma-colecao-o-que-fazer
locale: pt
legacySlug: nao-encontro-uma-colecao-o-que-fazer
---

> ⚠️ Existem duas formas de configurar coleções, por meio do CMS ou utilizando o módulo de Coleções Beta. Este artigo, refere-se à [configuração de coleções por meio do CMS](/pt/docs/tutorials/cadastro-de-colecoes-cms).

Por padrão, o CMS só exibe as __últimas 20 coleções__ atualizadas.

Portanto, se sua loja tem mais do que 20 coleções, é normal que algumas delas não apareçam na lista exibida no CMS.

![findCollection1](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/pt/faq/storefront/nao-encontro-uma-colecao-o-que-fazer_1.png)

Mas você pode buscar qualquer coleção utilizando o __campo de busca__ (Find), incluindo as que não são exibidas por padrão.

![findCollection2](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/pt/faq/storefront/nao-encontro-uma-colecao-o-que-fazer_2.png)
