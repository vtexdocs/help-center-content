---
title: 'Headless CMS (Legacy): nova solicitação de autorização'
...
A partir de 15 de Fevereiro, todos os [usuários](https://developers.vtex.com/docs/guides/api-authentication-using-user-tokens) e [chaves de aplicação](https://developers.vtex.com/docs/guides/api-authentication-using-application-keys) que precisarem acessar o Headless CMS (Legacy) deverão ter os [recursos](/pt/docs/tutorials/recursos-do-license-manager) `See CMS menu on the top-bar` e `Settings` do License Manager adicionados aos seus [perfis de acesso](/pt/docs/tutorials/perfis-de-acesso). Para o gerenciamento de conteúdo no Headless CMS (Legacy), também é necessário ter o [recurso](/pt/docs/tutorials/recursos-do-license-manager) `CMS GraphQL API`. Esse requisito se aplica à API do Headless CMS (Legacy) e ao Admin VTEX.

## O que mudou?
Antes, o gerenciamento de conteúdo da loja no Headless CMS (Legacy) podia ser feito por usuários e chaves de aplicação
sem ter um recurso específico do License Manager associado ao seu perfil de acesso.

Agora, qualquer solicitação via API do Headless CMS (Legacy) ou Admin VTEX autenticará o usuário e verificará se ele tem os recursos necessários associados ao seu perfil de acesso.
Os recursos `See CMS menu on the top-bar` e `Settings` concedem acesso ao Headless CMS (Legacy). Para o gerenciamento de conteúdo, os usuários também precisam do recurso `CMS GraphQL API`. Sem este recurso, não será possível gerenciar o conteúdo no Headless CMS (Legacy).

## Por que fizemos esta mudança?
Essa alteração visa aumentar a segurança e simplificar o acesso do usuário ao Headless CMS (Legacy), proporcionando aos usuários um ambiente mais controlado e seguro para gerenciar o conteúdo da loja.

## O que precisa ser feito?
Certifique-se de que os usuários do Headless CMS (Legacy) estejam associados aos recursos `CMS GraphQL API`, `See CMS menu on the top-bar` e `Settings` em seus perfis de acesso, seja [criando um novo perfil de acesso](#criar-um-novo-perfil-de-acesso) ou [editando um existente](#editar-um-perfil-de-acesso).
...
3. Clique no **nome do perfil** atribuído aos usuários do Headless CMS (Legacy).
...
<div style="background-color:#cfe2ff; border-left: 2px solid #084298; border-top-left-radius: 2px; border-bottom-left-radius: 2px; margin-bottom: 10px; padding: 15px">
  A opção de criar perfis de acesso personalizados pode variar de acordo com a configuração da loja, e cada loja pode definir um perfil específico para os usuários do Headless CMS (Legacy).
</div>
