---
title: 'Nova interface para Vínculos de SKU'
id: 7qbbvVykje1Q9kHT3J4sZE
status: PUBLISHED
createdAt: 2022-07-26T00:17:05.202Z
updatedAt: 2022-08-01T10:00:49.322Z
publishedAt: 2022-08-01T10:00:49.322Z
contentType: updates
productTeam: Marketing & Merchandising
author: 1malnhMX0vPThsaJaZMYm2
slugEN: 2022-07-26-new-sku-bindings-interface
locale: pt
legacySlug: nova-interface-para-vinculos-de-sku
announcementImageID: 'undefined'
announcementSynopsisPT: 'Redesenhamos a página Vínculos de SKU para facilitar o gerenciamento da relação entre os SKUs de sellers e marketplaces.'
---

A VTEX possibilita que a sua loja atue tanto como [seller](https://help.vtex.com/pt/tutorial/configurar-seller-para-vender-em-marketplace-vtex--6g045OkRSjNpqhkExbQRlP) quanto como [marketplace](https://help.vtex.com/pt/tutorial/configurar-marketplace-vtex--7splyp5MqIyt2Iyz5jsNzb), o que permite reduzir custos, alcançar mais clientes e aumentar a variedade de produtos oferecidos. Nesse contexto, cada marketplace precisa estabelecer e manter uma relação direta entre o seu catálogo e os [SKUs](https://help.vtex.com/pt/tracks/catalogo-101--5AF0XfnjfWeopIFBgs3LIQ/3mJbIqMlz6oKDmyZ2bKJoA) oferecidos por sellers.

Para aprimorar essa experiência, redesenhamos a página de **Vínculos de SKU** no Admin VTEX. A nova interface permite acompanhar e gerenciar a relação de vínculo entre os SKUs do seller e do marketplace de forma mais simples e ágil.

![sku_binding_page_PT_final](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/pt/announcements/2022/2022-07-26-nova-interface-para-vinculos-de-sku_1.png)

## O que mudou?

A página **Vínculos de SKU** foi completamente redesenhada. Confira as principais mudanças:

* Acrescentamos filtros e opções de busca mais acessíveis e otimizados.
* Criamos um novo fluxo simplificado para vincular ou desvincular SKUs.
* Atualizamos o design da página para seguir o _design system_ do [Novo Admin VTEX](https://content.vtex.com/join-new-admin-beta-program-pt/?utm_source=announcement&utm_medium=help_center&utm_campaign=sku_bindings).

Para visualizar a página, entre no Admin VTEX e acesse **Marketplace > Sellers > Vínculos de SKU**.

## Por que fizemos essa mudança?

Atualizamos a página para simplificar a experiência de gerenciar vínculos de SKUs entre marketplaces e sellers, facilitando a atuação de lojas VTEX em cenários de vendas de produtos de terceiros.

## O que precisa ser feito?

Nenhuma ação é necessária, a atualização será automática para todas as lojas. É possível voltar à versão anterior da experiência a qualquer momento, clicando no aviso no topo da nova página.

Para mais informações sobre como aproveitar a interface, confira o artigo [Vínculos de SKU](https://help.vtex.com/pt/tutorial/vinculos-de-sku--1SmrVgNwjJX17hdqwLa0TX).

Esta página segue o design do [Novo Admin VTEX](https://help.vtex.com/pt/announcements/bem-vindo-ao-novo-admin-vtex--5tLPBodp6Xu03vYdyBTGTa), com um visual mais simples, moderno e fácil de usar. Para acessar mais páginas com essa experiência, além de um Admin aprimorado com dados acionáveis, [faça parte](https://content.vtex.com/join-new-admin-beta-program-pt/?utm_source=announcement&utm_medium=help_center&utm_campaign=sku_bindings) do programa Beta do Novo Admin VTEX.
