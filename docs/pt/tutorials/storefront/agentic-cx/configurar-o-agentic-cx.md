...
A página de **Configurações** do Agentic CX do Admin VTEX está dividida em duas seções:
...
Para refinar as instruções, clique em `Refinar instruções`. Na aba de **Instruções**, você consegue:
...
Para configurar agentes de carrinho abandonado no WhatsApp da sua loja.
