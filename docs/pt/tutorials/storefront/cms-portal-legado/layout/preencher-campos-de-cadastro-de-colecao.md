...
- __Highlight?__: essa opção deve ser marcada para que a coleção seja destacada na sua loja, através do controle `$product.HightLight`. Para entender melhor sobre o uso desse controle, acesse nosso artigo [Controles do template de prateleira](https://developers.vtex.com/docs/guides/shelf-template-controls).

- __Searchable?__: essa opção deve ser marcada para que a coleção possa ser retornada na [API de VTEX Search (Legado)](https://developers.vtex.com/docs/api-reference/search-api#get-/api/catalog_system/pub/products/search/-search-?endpoint=get-/api/catalog_system/pub/products/search/-search-). 
