...
**Scopes** são a funcionalidade da Conta da organização que permite às organizações compradoras restringirem os atributos de **Unidades Organizacionais**. Isso permite às empresas controlarem quais informações os compradores de uma unidade de negócio podem ver e usar, limitando o acesso dos usuários ao necessário para suas funções.

Cada unidade organizacional possui um scope próprio, que não é herdado da unidade organizacional à qual está subordinada e sim configurado individualmente. Veja um exemplo na imagem a seguir, que representa a totalidade de dimensões da Conta da organização pelo conjunto ABC e mostra três unidades filhas:
...
- **Unidade organizacional 1:** não tem um scope configurado e, portanto, dispõe de todas as dimensões da Conta da organização.
- **Unidade organizacional 2:** tem um scope configurado e dispõe somente da dimensão A.
- **Unidade organizacional 3:** tem um scope configurado e dispõe somente das dimensões B e C.
...
Para visualizar e gerenciar o scope de uma unidade organizacional, o perfil de [Storefront Roles](https://developers.vtex.com/docs/guides/storefront-roles) do usuário deve ter os recursos `View_Organization_Unit` e `Edit_Organization_Unit`.
