...
1. Na [tela inicial da Organization Account](https://help.vtex.com/pt/docs/tutorials/organization-account-pt#acessando-a-organization-account), na seção **Finance and Compliance**, clique em `Buying Policies`.
...
3. No modal **Add new buying policy**, preencha os campos, que são todos obrigatórios:
    ...
    * Em **Criteria**, defina os critérios que resultam na aplicação da buying policy. Para isso, preencha o campo com uma expressão no formato [JSONata](https://jsonata.org/), tal como `value > 10000` (pedidos acima de R$ 100,00). É possível selecionar o critério desejado e apenas editar os valores desejados (destacados em roxo):
