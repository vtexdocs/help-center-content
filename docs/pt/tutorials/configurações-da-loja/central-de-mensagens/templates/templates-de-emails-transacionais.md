...
| Buyer Portal Sequential approval required (Approver) | buyer-portal-approver | Notificação enviada ao aprovador quando um pedido requer aprovação. | Quando um pedido não atende às políticas comerciais estabelecidas pela organização, ele precisa ser aprovado manualmente pelo aprovador. O status do pedido permanece em `waiting-for-authorization` até que seja aprovado ou rejeitado. |
| Buyer Portal Order Denied (Buyer) | buyer-portal-order-denied-buyer | Notificação enviada ao comprador de que o pedido foi rejeitado. | O aprovador recusou o pedido segundo as políticas da organização, atualizando o status do pedido para `rejected`. |
