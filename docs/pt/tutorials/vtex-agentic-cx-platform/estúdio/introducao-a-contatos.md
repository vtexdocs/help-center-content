...
- Através do botão **importar contatos** que permite a importação de uma planilha (que deve seguir regras específicas) com contatos que ainda não interagiram com a plataforma. Para acessar essas informações entre no seu projeto e acesse **Contatos** (em **Operações**) no menu lateral esquerdo.

Ao selecionar a opção **Contatos**, você irá se deparar com as seguintes funcionalidades:

- **Importar Contatos**: Botão para importar uma planilha (com algumas regras de construção) com contatos externos a plataforma. Se o usuário estiver nessa planilha, o VTEX Agentic CX Platform o reconhece e consegue tratá-lo pelo nome cadastrado desde a primeira mensagem enviada pela plataforma.
...
- **Gerenciar Campos**: Para criar campos de contato, onde conseguimos criar os campos variados para informações do contato.
- **Exportar**: Permite exportar todos os contatos da organização.
