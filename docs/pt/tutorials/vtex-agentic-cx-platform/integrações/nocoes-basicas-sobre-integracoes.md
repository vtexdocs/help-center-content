...
- **Nativa**: O VTEX Agentic CX Platform consegue integrar com APIs que tem formato REST/Json através de um **cartão Webhook** ou através de uma aplicação pré-existente no menu de Integrações.
...
Webhook é uma tecnologia utilizada para permitir a comunicação entre duas aplicações em tempo real. No VTEX Agentic CX Platform podemos buscar e até mesmo atualizar informações em outros sistemas através do cartão **Chamar Webhook**. Isso quer dizer que, para consumir uma API, você irá utilizar um cartão Webhook:
...
Nessa imagem podemos ver a documentação da API do VTEX Agentic CX Platform:
...
A documentação da API funciona como um manual que nos ajuda a entender como integrar. Nela será informado se as requisições são na arquitetura nativa do VTEX Agentic CX Platform: REST/Json.
...
## Como visualizar a chamada de API no VTEX Agentic CX Platform?
