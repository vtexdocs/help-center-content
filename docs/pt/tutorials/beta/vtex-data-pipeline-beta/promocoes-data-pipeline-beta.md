...
- [Tabela promotions_latest](#tabela-promotions_latest)  
- [Tabela promotions_historical](#tabela-promotions_historical)
...
- A chave de ligação é `id_calculator_configuration` (combinada com `account`). Pedidos e inventário entram na análise via correlação com outros conjuntos de dados.
...
flowchart TB
    subgraph ATUAL["Estado atual"]
        latest["promotions_latest<br/>(configuração vigente)"]
    end

    subgraph HIST["Histórico"]
        hist["promotions_historical<br/>(versões e alterações)"]
    end

    latest -->|"id_calculator_configuration"| hist
    latest -->|"promo aplicada no pedido"| ORD["Modelo de dados<br/>de Pedidos<br/>orders_rateandbenefitsidentifiers"]
    latest --> INV["Modelo de dados<br/>de Inventário"]
    latest --> NAV["Modelo de dados<br/>de Navegação"]
```
