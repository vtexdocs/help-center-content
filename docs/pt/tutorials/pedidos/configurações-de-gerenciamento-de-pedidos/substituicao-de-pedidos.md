...
A [Alteração de itens](/pt/docs/tutorials/alteracao-de-itens-de-um-pedido-finalizado) é diferente da substituição de pedidos. Na substituição de pedido o cliente solicita as mudanças no pedido, e na alteração de itens a loja modifica o pedido.
...
> ❗ Se a Substituição de pedidos for ativada em lojas que têm customização na página Meus Pedidos, é preciso recriar a customização. A funcionalidade substitui a versão antiga da página (que está customizada) por uma página nova não customizada. Para esses casos, recomendamos que, antes de ativar a funcionalidade na sua loja, utilize um [ambiente beta](/pt/docs/tutorials/acessar-o-ambiente-beta-pelo-dominio-myvtex-com) e valide a ativação com customização.
...
* Endereço de entrega;
* [Tipo de entrega](/pt/docs/tutorials/como-funciona-o-tipo-de-envio);
* Forma de pagamento;
* [SKU](/pt/docs/tutorials/o-que-e-um-sku);

### Restrições para as condições

Não é possível realizar substituição de pedidos nos casos de:

* [Pedidos incompletos](/pt/docs/tutorials/entendendo-os-pedidos-incompletos): quando o pedido não está com todas as informações completas para ser finalizado na plataforma.
...
> ⚠️ O aproveitamento do pagamento (utilizar o pagamento efetuado anteriormente) só pode ser usado se o valor do pedido novo for igual ao do pedido original. Se o pagamento anterior por cartão de crédito não for aproveitado, a cobrança anterior é automaticamente estornada. Para formas de pagamento que expiram em até 2 dias (como boleto), o pagamento não pode ser reutilizado.
