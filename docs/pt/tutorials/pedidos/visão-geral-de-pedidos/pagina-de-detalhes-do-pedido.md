...
A página de **Detalhes do Pedido** permite visualizar todas as informações de um pedido e gerenciá-lo em um único lugar. Para visualizá-la, no Admin VTEX, acesse **Pedidos > [Todos os Pedidos](/pt/docs/tutorials/todos-os-pedidos)**, e clique no pedido desejado.
...
Para saber mais sobre os status e o fluxo do pedido, confira o artigo [Fluxo e status de pedidos](/pt/docs/tutorials/fluxo-e-status-de-pedidos).
...
* **Status do pedido:** status em que o pedido se encontra no fluxo de pedidos. Confira o artigo [Fluxo e status de pedidos](/pt/docs/tutorials/fluxo-e-status-de-pedidos) para mais informações.
* **Ações:** menu que permite acessar ações rápidas referentes ao pedido.
    ...
    * **Faturar:** botão que permite [faturar um pedido](/pt/docs/tutorials/faturar-um-pedido-manualmente), aplicável a pedidos que ainda não foram faturados.
...
Este bloco apresenta o status atual do pedido no [fluxo de pedidos](/pt/docs/tutorials/fluxo-e-status-de-pedidos) VTEX. Nele você encontra o [Ver histórico](#historico-do-pedido), para visualizer todas as mudanças e interações realizadas no pedido.
...
* **Devolver itens:** ao clicar em `Devolver itens`, uma nova página com as informações da devolução é aberta. Para saber mais, consulte o artigo [Como devolver itens do pedido](/pt/docs/tutorials/como-devolver-itens-do-pedido).
* **Rastrear a entrega do pedido:** ao clicar em `Ver rastreio`, uma nova página é aberta com os detalhes da entrega.
* **Confirmar entrega:** confirma a entrega do pedido ao destinatário. Para confirmar a entrega, clique em <i class="fas fa-ellipsis-v"></i> `Ações` e selecione <i class="fas fa-check"></i> `Confimar entrega`. A confirmação de entrega não altera nenhum status ou [fluxo do pedido](/pt/docs/tutorials/fluxo-e-status-de-pedidos). A informação é usada apenas para indicar para a loja e o cliente que a entrega foi finalizada.
* **Alterar dados de rastreio:** inserir e [editar os dados de rastreamento](/pt/docs/tutorials/alterar-dados-de-rastreio) para uso da loja e do cliente.
...
* **Confirmar entrega:** confirma o recebimento do pacote devolvido. Saiba mais sobre a devolução de itens em [Como devolver itens do pedido](/pt/docs/tutorials/como-devolver-itens-do-pedido).
...
Para mais informações sobre quando o manuseio deve ser feito, leia o artigo [Fluxo e status de pedidos](/pt/docs/tutorials/fluxo-e-status-de-pedidos).
...
* **Valor total do pedido:** valor total cobrado pelo pedido, ou seja, soma do valor dos itens, do envio e dos serviços adicionais, se houver. 
    * **Alterar valor final:** permite [alterar o valor total cobrado](/pt/docs/tutorials/alterar-valor-total-do-pedido). 
