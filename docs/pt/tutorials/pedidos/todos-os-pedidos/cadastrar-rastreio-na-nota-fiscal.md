---
title: 'Cadastrar rastreio na nota fiscal'
id: 5acsOh2bssscYgmEKc8wK8
status: PUBLISHED
createdAt: 2017-08-04T14:10:31.584Z
updatedAt: 2023-04-28T23:41:47.471Z
publishedAt: 2023-04-28T23:41:47.471Z
firstPublishedAt: 2017-08-04T14:22:23.591Z
contentType: tutorial
productTeam: Post-purchase
author: authors_84
slugEN: register-tracking-on-the-tax-receipt
legacySlug: cadastrar-rastreio-na-nota-fiscal
locale: pt
subcategoryId: 2t00bBkcAwIkgSCGKsMOwY
---

Para cadastrar o rastreio na nota fiscal, siga os passos abaixo:

1. No Admin VTEX, acesse **Pedidos > Todos os pedidos**, ou digite **Todos os pedidos** na barra de busca no topo da página.
2. Selecione o pedido desejado.
3. Na seção **Itens a faturar**, clique no botão `Faturar`.
4. Selecione a opção desejada:
    - Enviar nota fiscal referenciando itens
    - Enviar nota fiscal avulsa
5. Ative o toggle <i class="fas fa-toggle-on"></i> **Rastreamento do pedido**.
6. Preencha os seguintes campos:
    - Número de rastreio
    - URL de rastreio
    - Transportadora
7. Clique em `Salvar nota fiscal`. 

> ℹ️ No momento de cadastrar o rastreio na nota fiscal, é possível alterar a [transportadora](/pt/docs/tutorials/transportadoras-na-vtex) associada ao pedido. Para isso na etapa de preenchimento do **Rastreamento do pedido**, clique em *Transportadora* e selecione a opção desejada.
