...
O fluxo de pedidos tem uma[ janela para cancelamento](/pt/docs/tutorials/fluxo-e-status-de-pedidos) (carência) na qual o cliente pode cancelar automaticamente um pedido. Com exceção desse período, o seller pode recusar uma solicitação de cancelamento do pedido, independente se ela foi realizada pelo cliente ou pelo marketplace. 
...
> ℹ️ Neste artigo, os termos marketplace e seller se referem ao [marketplace VTEX](/pt/docs/tutorials/estrategias-de-marketplace-na-vtex#ser-um-marketplace-vtex) e ao [seller VTEX](/pt/docs/tutorials/estrategias-de-marketplace-na-vtex#sendo-seller-vtex). A funcionalidade de recusar solicitação de cancelamento não está disponível para sellers e marketplaces externos.
...
1. O cliente entra em contato com o marketplace para cancelar um pedido ou o faz através da [Minha Conta](/pt/docs/tutorials/como-funciona-a-minha-conta).
2. O marketplace envia uma solicitação de cancelamento ao seller. Nesta etapa, o [status do pedido](/pt/docs/tutorials/fluxo-e-status-de-pedidos) para o marketplace é `aguardando decisão do seller`. 
...
A solicitação de cancelamento de um pedido pode ser feita pelo marketplace ou pelo cliente através da [Minha conta](/pt/docs/tutorials/como-funciona-a-minha-conta). Quando feita diretamente pela **Minha conta,** o cliente recebe uma mensagem de confirmação de que a solicitação foi enviada com sucesso. 

Depois disso, o pedido permanecerá com o [status](/pt/docs/tutorials/fluxo-e-status-de-pedidos) de `cancelamento solicitado` para o marketplace e `aguardando decisão do seller` para o seller até que ele aceite ou não a solicitação. 
...
1. No Admin VTEX, acesse **Pedidos > Todos os pedidos** ou digite [Todos os pedidos](/pt/docs/tutorials/todos-os-pedidos) na barra de busca no topo da página.
2. Selecione o pedido desejado para acessar a página [Detalhes do pedido](/pt/docs/tutorials/pagina-de-detalhes-do-pedido).
...
4. Preencha o pop-up **Solicitação de cancelamento** com o motivo da recusa da solicitação de cancelamento do pedido.
  ...
  > ⚠️ Lembre-se de que a mensagem contida nesse campo poderá ser exibida ao cliente pela [Minha conta](/pt/docs/tutorials/como-funciona-a-minha-conta) ou enviada pelo marketplace.
