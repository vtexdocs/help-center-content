---
title: 'Vendedor temporário'
id: 1D0znuLfmKpcuYGp29qrEX
status: PUBLISHED
createdAt: 2025-09-16T16:41:13.120Z
updatedAt: 2025-09-16T16:52:04.541Z
publishedAt: 2025-09-16T16:52:04.541Z
firstPublishedAt: 2025-09-16T16:51:17.205Z
contentType: tutorial
productTeam: Others
author: 2AhArvGNSPKwUAd8GOz0iU
slugEN: temporary-seller
legacySlug: vendedor-temporario
locale: pt
subcategoryId: 1acPW0OHXq4PjbQUFVrNrY
---

Para cadastrar um vendedor temporário, siga as etapas abaixo:

<ol>
  <li>No menu lateral, acesse **Cadastro &gt; Vendedores**.</li>
  <li>Clique no botão azul **Cadastrar Vendedor**.</li>
  <li>Preencha as informações do vendedor.</li>
  <li>No campo **Tipo de contratação**, selecione **Temporário**.</li>
  <li>Informe a **Data de início** e a **Data de término**.</li>
</ol>

![Exemplo de seleção do tipo temporário e datas](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/pt/tutorials/indeva-by-vtex/vendedores/vendedor-temporario_1.png)

<ol start="6">
  <li>Clique em **Cadastrar Vendedor** para finalizar o processo.</li>
</ol>

