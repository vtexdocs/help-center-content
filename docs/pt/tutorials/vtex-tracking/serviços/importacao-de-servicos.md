...
* [Aplicativo VTEX Tracking](/pt/docs/tutorials/como-instalar-e-configurar-a-app-vtex-tracking-no-seu-admin-vtex)
...
* **ID do cliente:** Identificação do cliente. Essa identificação é criada pela plataforma web em [Configurações de conta > Clientes.](/pt/docs/tutorials/clientes-vtex-tracking) O uso desse campo substitui o preenchimento do nome do cliente, CPF e os dados referentes ao endereço do cliente.
* **ID do parceiro:** Identificação do parceiro. Essa identificação é criada pela plataforma web em [Configurações de conta > Parceiros.](/pt/docs/tutorials/parceiros-vtex-tracking) Utilize para facilitar o acesso do parceiro às informações do serviço.
...
* **Portador:** o usuário criado para o portador - motorista - utilizar do aplicativo. O campo deve ser preenchido com o usuário que é utilizado pelo portador para o seu acesso no aplicativo VTEX Tracking. Há dois casos de uso desse campo:
    ...
    * Se não preenchido: o serviço será cadastrado sem portador atribuído, então utilize o[ cadastro de rota](/pt/docs/tutorials/nova-rota) para definir um portador e para otimizar a rota criada. 
