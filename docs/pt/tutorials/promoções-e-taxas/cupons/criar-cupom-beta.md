...
* **Promoções vinculadas:** exibe o nome das promoções vinculadas ao cupom, se houver alguma. Ao clicar no nome de uma promoção, você será direcionado para a página de edição da promoção.
    ...
    O cupom só se torna disponível para uso quando está vinculado a pelo menos uma promoção. Para estabelecer esse vínculo, é necessário usar a mesma [utm_source ou utm_campaign](/pt/docs/tutorials/o-que-sao-utm-source-utm-campaign-e-utm-medium) no cadastro do cupom e no cadastro da promoção. Leia a seção [Vincular o cupom a uma promoção](#vincular-o-cupom-a-uma-promocao) para saber como fazer essa configuração.
...
É obrigatório vincular um cupom a uma promoção para ativá-lo. Para isso, siga os passos abaixo. É possível associar um cupom a qualquer tipo de promoção, exceto a [Promoção de campanha](/pt/docs/tutorials/promocao-de-campanha).
