...
A configuração da integração com o Magalu é feita em duas etapas: primeiro a configuração no Admin VTEX e, depois, a autenticação no [portal do Magalu Marketplace](https://marketplace-vendamais.magazineluiza.com.br/). Antes de iniciar, certifique-se de estar [cadastrado](/pt/tracks/configurar-integracao-com-o-magazine-luiza--5Yx5IrNa7Y48c6aSC8wu2Y) no portal do Magalu Marketplace e siga as instruções abaixo:
...
|---|---|---|
| **Integração*** | Marque como *Ativada* para que a integração comece a operar. Se optar por desativar a integração, toda a configuração, incluindo o afiliado, será excluída. | ![Integração Magalu](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/pt/tracks/canais-de-venda/marketplace/configurar-integracao-com-o-magazine-luiza/configurar-integracao_1.png) |
| **CNPJ*** | CNPJ utilizado no cadastro no portal Magalu Marketplace. Inserir apenas números, sem caracteres especiais. | **45121594000127** |
...
| **Estoque Mínimo*** | Se a quantidade de itens de um produto em estoque estiver abaixo do valor escolhido, seu anúncio é pausado para evitar que sejam feitas vendas sem estoque durante o tempo de atualização do Magalu pela VTEX. O valor de estoque mínimo é aplicado a todos os produtos, portanto verifique se algum produto deixará de ser anunciado devido ao seu estoque inicial. Se o saldo do SKU estiver igual ou abaixo do valor configurado, será enviado saldo 0 para o marketplace. | **3** |
| **Ativar estoque para entrega e cotação de frete para contas franquias** | Com a opção _Sim_ selecionada, a funcionalidade [Multilevel Omnichannel Inventory](/pt/docs/tutorials/multilevel-omnichannel-inventory) permite vender no Magalu produtos do estoque das [contas franquia](/pt/docs/tutorials/o-que-e-conta-franquia). Os pedidos podem ser roteados para as franquias no fluxo de pedido em cadeia. Esta configuração está em [open beta](/pt/docs/tutorials/ship-from-store-para-marketplaces). | _Sim_ |
| **Percentual de tolerância na divergência do valor do pedido** | Taxa de aceitação da diferença entre o preço do Magalu e o preço determinado pelo seller. Se a divergência, em percentual, for menor ou igual ao valor definido, o pedido é integrado. Caso contrário, o pedido não é integrado e pode ser acompanhado em **Marketplace > Conexões > Pedidos**. Em pedidos em cadeia, esta é a tolerância que vale para o canal Magalu; a [regra global de Divergência de valores](/pt/docs/tutorials/regra-de-divergencia-de-valores) não se aplica a esse fluxo. Saiba mais em [Ship from store para marketplaces](/pt/docs/tutorials/ship-from-store-para-marketplaces). | **10** |
| **Mapeamento de SLA Customizável** | Tipo de Entrega Magalu x Tipo de Entrega VTEX. O campo busca automaticamente a [política de envio](/pt/docs/tutorials/politica-de-envio) pelo nome cadastrado no Magalu e, se encontrar uma correspondência com uma política de envio da sua loja na VTEX, a associação entre elas é realizada. Caso os nomes das políticas sejam distintos, é necessário fazer a correspondência no card da integração. No campo **De**, adicione o nome da política de envio do Magalu e, no campo **Para**, adicione o nome da política de envio na VTEX. Esteja atento a letras maiúsculas, minúsculas e espaçamentos. | **MAGALU EXPRESSO 24H > Entrega rápida**|
| **Enviar Kit*** | Ao escolher Não, SKUs [cadastrados como kit](/pt/docs/tutorials/cadastrar-kit) não são enviados para o Magalu Marketplace. Ao habilitar Sim, todos os itens da sua política comercial são enviados para o marketplace, sem filtrar SKUs cadastrados como kit. Mudanças na configuração do conector podem ser feitas a qualquer momento, sendo que isso não afeta SKUs que já tenham sido enviados. Por exemplo, se você mudar sua configuração de *Sim* para *Não*, SKUs enviados anteriormente para o Magalu não são removidos ou alterados. Com a opção Não ativada, tentativas de envio para o Magalu Marketplace de SKUs cadastrados como kit resultam no seguinte aviso: `O conector está configurado para não enviar SKUs do tipo Kit.`  Esta mensagem pode ser visualizada no Admin VTEX, em** Marketplace > Conexões > Produtos**, clicando no SKU. | ![Enviar kit Magalu](https://cdn.statically.io/gh/vtexdocs/help-center-content/refs/heads/main/docs/pt/tracks/canais-de-venda/marketplace/configurar-integracao-com-o-magazine-luiza/configurar-integracao_2.png) |
...
|**Etiqueta de entrega**| Qual tipo de arquivo da etiqueta de entrega deve ser disponibilizado. | PDF ou ZPL |
...
1. Preencha o campo DE com o nome do campo de origem: `ProductName` para título do produto ou `ProductDescription` para descrição do produto.  
...
3. Clique no botão **+** e uma regra de mapeamento será adicionada à sua integração.  
