---
title: 'VTEX Live Shopping'
id: 17HV0OAaCIB46tW9O1rVEJ
status: PUBLISHED
createdAt: 2025-07-23T17:25:26.276Z
updatedAt: 2025-09-06T18:12:41.458Z
publishedAt: 2025-09-06T18:12:41.458Z
firstPublishedAt: 2025-07-28T15:07:19.474Z
contentType: trackArticle
productTeam: Marketing & Merchandising
slugEN: vtex-live-shopping
locale: pt
trackId: 3eCHNCIx8XhZOSmX0sQT3U
trackSlugEN: vtex-live-shopping-primeiros-passos
order: 1
---

O aplicativo [Live Shopping](https://help.vtex.com/pt/tutorial/live-shopping--1cYWPIbjNMyr072sksHSVL) permite que as lojas criem experiências de compra imersivas e em tempo real diretamente do storefront. Os lojistas podem transmitir eventos ao vivo no site da loja para promover coleções de produtos, aumentar as vendas e melhorar a experiência do usuário, enquanto interagem por meio do chat ao vivo.

Este guia técnico de onboarding foi desenvolvido para auxiliar as equipes de desenvolvimento e operações na implementação, configuração e otimização do **Live Shopping**. O objetivo é garantir uma configuração sem fricção e permitir que as marcas realizem eventos ao vivo que sejam envolventes e gerem conversões.

## Visão geral

Em um evento de **Live Shopping**, os seguintes recursos estão disponíveis:

* **Reprodutor de mídia**: exibe a transmissão com opções como tela cheia, curtidas, compartilhamento e picture-in-picture (PiP), que seria uma janela flutuante.​ ​
* **Barra lateral de produtos**: exibe até 50 produtos durante o evento, com a possibilidade de colocá-los em destaque.
* **Chat**: permite a interação em tempo real entre administradores e clientes da loja.
* **Botão de curtir:** mostra uma animação de coração quando os clientes interagem com um "curtir".
* **Opções de compartilhamento:** permite que a transmissão ao vivo seja compartilhada por qualquer participante.
* **Tela inteira:** exibe o evento no modo de tela inteira.
* **Número de espectadores:** mostra quantos espectadores estão assistindo ao evento.

Veja abaixo como os recursos apareceriam em um evento real de **Live Shopping**:

![article_1_overview_v2_PT](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/pt/tracks/omnichannel/vtex-live-shopping-primeiros-passos/vtex-live-shopping_1.png)

> ℹ️ O [Live Shopping](https://help.vtex.com/en/tutorial/live-shopping--1cYWPIbjNMyr072sksHSVL) é um aplicativo pago com diferentes planos de assinatura. Para mais informações, acesse a [VTEX App Store](https://apps.vtex.com/liveshopping/p).

Em caso de dúvidas, comentários ou ajuda com o **Live Shopping**, entre em contato com nosso time de [Suporte](https://docs.google.com/forms/d/e/1FAIpQLSf2PDBBWX2hXzw7Pa7VAn9l49CDUpI4Ug1U6yshDdas2d_Nkg/viewform).

