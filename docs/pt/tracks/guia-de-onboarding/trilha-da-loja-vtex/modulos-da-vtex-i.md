...
- __Tornar produtos disponíveis para clientes:__ para [disponibilizar um produto na loja](/pt/faq/por-que-o-produto-nao-aparece-no-site), após as configurações de Catálogo, é preciso que o SKU tenha [preço](#precos) cadastrado e [logística](#logistica) para envio. Além disso, o CMS da loja precisa estar configurado corretamente.
...
| Kits | Um kit é um conjunto de SKUs que são vendidos juntos, ou seja, um SKU composto por um ou mais SKUs. | __Help Center__ <br> - [O que é um kit?](/pt/docs/tutorials/o-que-e-um-kit) <br> - [Cadastrar kit](/pt/docs/tutorials/cadastrar-kit) <br> __Developer Portal__ <br> - [Catalog API - SKU Kit](https://developers.vtex.com/vtex-rest-api/reference/catalog-api-get-sku-kit) |
...
Preços podem ser agendados para eventos especiais, como descrito no artigo [Agendar preço](/pt/docs/tutorials/agendar-preco).

### Relação entre tabelas de preços e políticas comerciais

Há uma diferença entre as tabelas de preços criadas para [políticas comerciais](/pt/docs/tutorials/como-funciona-uma-politica-comercial) e tabelas de preços criadas para outros contextos.
...
É possível criar cupons de desconto pelo [Admin VTEX](/pt/docs/tutorials/criar-cupom-beta) ou por [API](https://developers.vtex.com/docs/guides/creating-and-managing-coupons-with-promotions-api). Para ser utilizado, um cupom precisa estar vinculado a pelo menos uma promoção. 
...
Antes de criar uma promoção de campanha, é necessário criar uma [audiência de campanha](/pt/docs/tutorials/criar-audiencia-de-campanha) para definir o público alvo da promoção. Assim, é possível selecionar segmentos de clientes elegíveis na promoção, que funcionam como gatilho, a partir dos critérios definidos pelo lojista.

Uma vez que a audiência de campanha foi criada, é preciso associá-la a uma [Promoção Regular](/pt/docs/tutorials/promocao-regular-pt), criando assim uma promoção de campanha. Esse tipo de promoção engloba todas as possibilidades presentes na [Promoção Regular](/pt/docs/tutorials/promocao-regular-pt) e algumas opções a mais, que são:
...
Na VTEX, o módulo [Logística](/pt/tracks/logistica-101--13TFDwDttPl9ki9OXQhyjx) é o responsável pela [logística e o fulfillment](/pt/docs/tutorials/fulfillment-logistica-vtex) de pedidos da loja, incluindo planejamento, transporte, armazenamento de produtos e envio de pedidos a clientes.
...
O conjunto de configurações essenciais de logística de uma loja VTEX é chamado de [Estratégia de envio](/pt/docs/tutorials/estrategia-de-envio), composta de três conceitos principais: política de envio, estoque e doca.
...
| [Inventário](#gerenciamento-de-inventario) | [Gerenciamento de inventário](/pt/docs/tutorials/gerenciar-itens-em-estoque) | [Update inventory by SKU and warehouse](https://developers.vtex.com/docs/api-reference/logistics-api#put-/api/logistics/pvt/inventory/skus/-skuId-/warehouses/-warehouseId-) |

#### Política de envio 

[Política de envio](/pt/docs/tutorials/politica-de-envio) é um conjunto de regras que definem as opções e condições de envio que serão apresentadas aos clientes no checkout. As condições de envio incluem transportadora, prazos de entrega, custo de frete, entre outros. A política de envio é vinculada ao estoque pela doca.
...
É a partir do [estoque](/pt/docs/tutorials/estoque) que se inicia a rota logística. Ele corresponde ao espaço físico de armazenamento das mercadorias vendidas pela loja. Uma vez que os itens vendidos são separados do estoque, eles são encaminhados para a doca.
...
Para [calcular o prazo de envio](/pt/docs/tutorials/como-funciona-o-calculo-de-envio) de pedidos, a plataforma VTEX considera as seguintes configurações:
...
- [Lead time](/pt/docs/tutorials/lead-time-tempo-de-envio-a-nivel-de-sku): quantidade de tempo configurável para o SKU de um estoque. A configuração é opcional e, por padrão, está definida como zero dias.
- [Tempo de estoque](/pt/docs/tutorials/gerenciar-estoque): tempo que o SKU leva para sair de um estoque para uma [doca](/pt/docs/tutorials/doca). Essa configuração é obrigatória, mas o tempo pode ser definido igual a zero.
- [Tempo de doca](/pt/docs/tutorials/gerenciar-doca): tempo que o SKU leva para sair da doca para ser enviado para o cliente. Essa configuração é obrigatória, mas o tempo pode ser definido igual a zero.
- [Política de envio](/pt/docs/tutorials/politica-de-envio): agrupa as regras e condições de envio apresentadas aos clientes no checkout, incluindo transportadoras, custo de envio, horário de funcionamento da transportadora, janelas de entrega e capacidade de entrega.

O [simulador de envio](/pt/docs/tutorials/simulador-de-envio) permite ao lojista simular e analisar as opções de entrega disponíveis para um pedido em uma localidade. A simulação verifica a viabilidade e as condições de envio do pedido, tais como disponibilidade de itens, formas de envio, custos e prazos.
