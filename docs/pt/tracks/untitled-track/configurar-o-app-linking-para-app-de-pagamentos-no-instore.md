...
![Payment app](https://raw.githubusercontent.com/vtexdocs/help-center-content/refs/heads/main/docs/pt/tracks/untitled-track/configurar-o-app-linking-para-app-de-pagamentos-no-instore_1.png)
...
- `acquirerId`: string - ID de afiliação do adquirente que está registrado no [Pagamentos da VTEX](/pt/tutorial/configurar-o-app-linking-para-pagamentos-no-instore#configurar-conector-do-instore-no-pci-gateway-da-vtex). Ex: `<stone_code>`, `<sitef_storeId>`.
...
>ℹ️ Para criar qualquer configuração extra, você precisa enviar para a equipe do inStore (instoredevs@vtex.com.br) os extras que a app precisa para concluir a transação. Assim, criaremos um formulário no Pagamentos para que o cliente possa configurar seu adquirente.
...
- `firstDigits`: string - BIN / IIN do cartão (seis primeiros números) - opcional
- `lastDigits`: string - quatro últimos números do cartão - opcional
- `cardToken`: string - token do cartão utilizado
