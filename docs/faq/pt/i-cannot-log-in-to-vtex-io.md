---
title: Não consigo efetuar login no VTEX.IO
id: 1YVcrUfVra2cKogEsWS0I2
status: DRAFT
createdAt: 2018-06-15T19:21:51.893Z
updatedAt: 2020-03-13T21:25:28.341Z
publishedAt: 
firstPublishedAt: 2018-06-15T19:24:32.575Z
contentType: frequentlyAskedQuestion
productTeam: VTEX IO
author: TnXcuQydAAOuwWACo864E
slug: nao-consigo-efetuar-login-no-vtex-io
legacySlug: nao-consigo-efetuar-login-no-vtex-io
---

Caso receba mensagem de erro `error: Connection to login server has failed` e não consiga conectar com sucesso ao VTEX.IO, indicamos que revise as permissões de seu Firewall.
