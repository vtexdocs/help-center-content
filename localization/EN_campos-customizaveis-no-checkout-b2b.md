...
# Custom fields in B2B checkout

**Custom fields in B2B checkout** allow you to collect additional information from purchases in the **B2B Buyer Portal**, such as **cost center**, **internal purchase order (PO) number**, and other control information. This information is associated with the order, helping the company standardize information, apply internal rules, and facilitate audits.

> ⚠️ This feature is only available for the **B2B Buyer Portal** and requires authorization from the account's **commerce engineer**.

## Using custom fields

Custom fields allow your company to:
...
Custom fields involve two roles:
...
When creating a custom field, the admin must choose one of the following types:
...
- Address.
- Card.
- Custom fields of type `option`.

> ⚠️ Only fields of type `option` can be set as a default value.

> ℹ️ For more information on how to add or configure custom fields, see [Managing custom fields in B2B checkout](https://help.vtex.com/en/docs/tutorials/gerenciar-campos-customizaveis-no-checkout-b2b).
