...
> ⚠️ El módulo **Capacidad Operativa (Beta)** no gestiona la entrega de pedidos. Esta gestión la realiza el módulo [**Capacidade de entrega**](https://help.vtex.com/es/docs/tutorials/gestionar-capacidad-de-entrega).
...
> ℹ️ Para poder activar y gestionar el módulo **Capacidad operativa (beta)**, tu [rol](/es/docs/tutorials/roles) en el Admin VTEX debe estar asociado al [recurso de License Manager](/es/docs/tutorials/recursos-del-license-manager) llamado _Logistics full access_.
...
La página **Capacidad operativa (beta)** presenta la siguiente información:
...
> ℹ️ Puede asociarse la búsqueda de sellers con el [filtro por status](/es/docs/tutorials/capacidad-operacional-beta#filtrar-sellers). Si no se devuelve ningún resultado, comprueba que no haya ningún filtro activo.
...
Para desactivar el módulo **Capacidad operativa (Beta)**, siga estos pasos:
...
* **[Módulo desactivado](#desactivar-modulo-capacidad-operativa-beta):** cuando el módulo Capacidad operativa (beta) no está habilitado, el módulo no tiene efecto y no se puede gestionar la capacidad operativa de sellers. Sin embargo, al desactivar el módulo, la configuración de la capacidad operativa de los sellers realizada anteriormente no se pierde y vuelve a funcionar cuando se reactiva el módulo.
  > **Ejemplo:** por algún imprevisto en tu operación, prefieres dejar de utilizar temporalmente el módulo Capacidad operativa (beta). En este caso, sería apropiado desactivarlo.
...
> ⚠️ En cualquier momento puedes volver a [activar el módulo Capacidad operativa (beta)](es/docs/tutorials/capacidad-operacional-beta) para gestionar la capacidad de tus sellers.
