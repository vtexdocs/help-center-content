...
17. **Cross-portal term consistency** — when the same product appears in both the VTEX Help Center and the VTEX Developer Portal, both portals must use the same canonical label; diverging names (e.g., "Intelligent Search" vs "Search", "Shipping" vs "Logistics") produce competing retrieval results that confuse both human users and AI systems; intentional differences must be documented in `08-cross-portal-alignment.md`
