...
async function checkLinksInFile(content) {
  ...
  for (const link of links) {
    ...
    if (state === "invalid" || state === "unknown") {
      const issue = `${displayPath}:${link.lineNumber}:${link.columnNumber}:🚨 **Found a broken link in a ${link.type} (Error ${statusCode?statusCode:"unknown"}):**<br> ${link.url}<br><blockquote>👉 Please review this link before merging your Pull Request.</blockquote>`;
      issues.push(issue);
    }
  }
...
(async () => {
  try {
    ...
    if (issues.length > 0) {
      console.error(`\nBroken links found in ${displayPath}:`);
