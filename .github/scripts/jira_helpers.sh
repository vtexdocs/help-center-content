...
resolve_jira_reporter_from_github() {
  ...
  pick_jira_user_by_display_name() {
    local name="$1"
  ...
  try_reporter_from_email() {
    ...
    match_json=$(jira_user_search "$email" 5 | pick_jira_user_by_email "$email")
  ...
  match_json=$(echo "$candidates" | pick_jira_user)
  if [ -n "$match_json" ] && [ "$match_json" != "null" ]; then
    echo "$match_json" | reporter_field_from_user
  ...
  if [ -n "${GH_TOKEN:-}" ]; then
    ...
    if [ -n "$display_name" ]; then
      match_json=$(jira_user_search "$display_name" 20 | pick_jira_user_by_display_name "$display_name")
      if [ -n "$match_json" ] && [ "$match_json" != "null" ]; then
        echo "$match_json" | reporter_field_from_user
