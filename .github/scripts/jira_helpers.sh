...
resolve_jira_reporter_from_github() {
  ...
  if [ -n "${GH_TOKEN:-}" ]; then
    ...
    if [ -n "$email_query" ]; then
      match_json=$(jira_user_search "$email_query" 5 | pick_jira_user_by_email "$email_query")
      if [ -n "$match_json" ] && [ "$match_json" != "null" ]; then
        echo "$match_json" | reporter_field_from_user
  ...
  if [ -n "${LOC_JIRA_REPORTER_EMAIL_DOMAIN:-}" ]; then
    ...
    match_json=$(jira_user_search "$derived_email" 5 | pick_jira_user_by_email "$derived_email")
    if [ -n "$match_json" ] && [ "$match_json" != "null" ]; then
      echo "$match_json" | reporter_field_from_user
